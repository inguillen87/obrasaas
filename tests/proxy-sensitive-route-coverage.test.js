...
const SENSITIVE_API_PATHS = [
  '/api/execution/assignments/owners',
  ...
  '/api/project-certificates',
  '/api/project-certificates/certificate-a/decision',
...
test('every API Route Handler is exhaustively classified behind Clerk or its own authentication', async () => {
  ...
  assert.equal(protectedRouteCount, 134, 'the complete current Clerk surface must remain classified');
