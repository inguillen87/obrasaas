...
export function database(){
  ...
  const match=(row,where)=>Object.entries(where).every(([key,value])=>{
    ...
    if(value&&typeof value==='object'&&!(value instanceof Date))return Object.entries(value).every(([op,v])=>op==='in'?v.includes(row[key]):op==='not'?comparable(row[key])!==comparable(v):row[key]!=null&&(op==='lt'?comparable(row[key])<comparable(v):op==='gt'?comparable(row[key])>comparable(v):op==='gte'?comparable(row[key])>=comparable(v):false));
  ...
  const tx={
    ...
    task:{findMany:async options=>{assert.equal(options.where.projectId,scope.projectId);assert.equal(options.take,options.where.id.in.length);return options.where.id.in.map(id=>({id,projectId:scope.projectId,title:'Actividad '+id}));},findFirst:async({where})=>state.task&&where.id==='task-a'&&where.projectId===scope.projectId&&where.metadata.equals==='canonical-task-v1'&&(!where.type||where.type===state.task.type)?structuredClone(state.task):null},
    worker:{findFirst:async({where})=>state.owner&&where.id==='worker-a'&&where.projectId===scope.projectId&&(where.active===undefined||where.active===true)?{id:'worker-a',name:'Persona de ensayo',active:true}:null,
      findMany:async options=>{assert.equal(options.where.projectId,scope.projectId);if(options.where.id){assert.equal(options.take,options.where.id.in.length);return options.where.id.in.map(id=>({id,projectId:scope.projectId,name:'Persona de ensayo'}));}assert.equal(options.take,101);return Array.from({length:state.workerCount},(_,i)=>({id:i?'worker-'+i:'worker-a',name:'Persona '+i}));}},
    workTeam:{findFirst:async({where})=>state.team&&where.id==='team-a'&&where.projectId===scope.projectId&&(!where.status||where.status==='ACTIVE')?{id:'team-a',name:'Cuadrilla de ensayo',status:'ACTIVE'}:null,
      findMany:async options=>{assert.equal(options.where.projectId,scope.projectId);if(options.where.id){assert.equal(options.take,options.where.id.in.length);return options.where.id.in.map(id=>({id,projectId:scope.projectId,name:'Cuadrilla de ensayo'}));}assert.equal(options.take,101);return state.team?[{id:'team-a',name:'Cuadrilla Norte'}]:[];}},
