...
try{
 ...
 await openReply();await expect(reply.getByText(f.reply.body,{exact:true})).toBeVisible();const count=requests.length;await page.evaluate(()=>window.dispatchEvent(new CustomEvent('fixture-context',{detail:{online:false}})));await expect(reply).toHaveCount(0);await page.evaluate(()=>window.dispatchEvent(new CustomEvent('fixture-context',{detail:{online:true}})));await expect(reply).toHaveCount(0);assert.equal(requests.length,count);await expect(history.getByRole('listitem')).toHaveCount(0);await openHistory();await expect(reply).toHaveCount(1);assert.equal(requests.length,count+1);await expect(reply.getByText(f.reply.body,{exact:true})).toHaveCount(0);cases.push('offline-reconnect-requires-new-explicit-history-and-reply-read');
