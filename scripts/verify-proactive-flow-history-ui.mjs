...
try{
 ...
 await refresh();await expect(history.getByText('Respuesta registrada',{exact:true}).first()).toBeVisible();await history.getByRole('button',{name:'Revisar pendientes',exact:true}).click();await expect(history.getByRole('listitem')).toHaveCount(2);await history.getByRole('button',{name:'Todos en esta página (20)',exact:true}).click();
