...
const entry=`import React,{useState,useEffect}from'react';import{createRoot}from'react-dom/client';import History from'./src/app/dashboard/inbox/proactive-flow-history';import './src/app/globals.css';function App(){const[chat,setChat]=useState('conversation-a'),[online,setOnline]=useState(true);useEffect(()=>{const change=e=>{if(e.detail.chat)setChat(e.detail.chat);if(typeof e.detail.online==='boolean')setOnline(e.detail.online)};window.addEventListener('fixture-context',change);return()=>window.removeEventListener('fixture-context',change)},[]);return <main><p>PRUEBA CONTROLADA · NO ENVÍA MENSAJES</p><form aria-label="Redactor de ensayo" onSubmit={event=>{event.preventDefault();window.fixtureComposerSubmits=(window.fixtureComposerSubmits||0)+1;}}><History organizationId="organization-a" projectId="project-a" conversationId={chat} online={online}/><label>Respuesta de ensayo<textarea defaultValue="Borrador que no debe enviarse"/></label><button type="submit">Enviar borrador de ensayo</button></form></main>};createRoot(document.getElementById('root')).render(<React.StrictMode><App/></React.StrictMode>);`;
...
try{
 ...
 await search.press('Enter');assert.equal(requests.length,beforeTools);assert.equal(await page.evaluate(()=>window.fixtureComposerSubmits||0),0,'Search Enter must not submit the enclosing message composer');assert.equal(await page.locator('form form').count(),0,'Search must not nest a form inside the composer');
 ...
 cases.push('search-inside-composer-enter-no-submit-private-data-not-indexed-and-clear-focus');
 ...
 await search.fill('hormigon');await expect(history.getByRole('listitem')).toHaveCount(1);for(const width of [390,1280]){await page.setViewportSize({width,height:900});await history.scrollIntoViewIfNeeded();await history.screenshot({path:resolve(out,'search-'+width+'.png')});}await search.fill('');
 ...
 assert.deepEqual(errors,[]);assert.equal(requests.some(r=>r.method!=='GET'),false);assert.equal(await page.evaluate(()=>window.fixtureComposerSubmits||0),0);assert.equal(await page.locator('form form').count(),0);
 const proof={status:'PASS',environment:'real-history-component-HTTP-route-query-service-with-controlled-session-and-in-memory-database',cases,widths:[320,390,768,1280],pageErrors:0,networkWrites:0,composerSubmissions:0,nestedForms:0,filterQueries:0,providerCalls:0,realMessagesSent:0,clerkVerified:false};writeFileSync(resolve(out,'proof.json'),JSON.stringify(proof,null,2));console.log(JSON.stringify(proof));
