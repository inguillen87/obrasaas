...
try{
 ...
 mode='normal';await section.getByRole('button',{name:'Consultar incidencia vinculada',exact:true}).click();await expect(section).toContainText(f.incident.id);await page.evaluate(()=>window.dispatchEvent(new CustomEvent('fixture-context',{detail:{online:false}})));await expect(section).toHaveCount(0);const before=requests.length;await page.evaluate(()=>window.dispatchEvent(new CustomEvent('fixture-context',{detail:{online:true}})));await expect(section).toHaveCount(0);await expect(page.getByRole('button',{name:'Consultar respuesta vinculada',exact:true})).toBeVisible();assert.equal(requests.length,before);await page.getByRole('button',{name:'Consultar respuesta vinculada',exact:true}).click();await expect(section.getByRole('button',{name:'Consultar incidencia vinculada',exact:true})).toBeVisible();assert.equal(requests.length,before+1);assert.equal(requests.at(-1).mode,'reply');await expect(section.getByText(f.incident.id,{exact:true})).toHaveCount(0);cases.push('reconnect-requires-fresh-parent-reply-before-incident-read');
