...
function App(){const[prepared,setPrepared]=useState(null);const[pin,setPin]=useState('');return <main style={{maxWidth:1120,margin:'auto',padding:16}}>
<Setup organizationId="company-a" projectId="project-a" companyName="Constructora de ensayo" onState={setPrepared}/>
<Connect companyName="Constructora de ensayo" projectName="Obra Norte" linked={false} configured={true} sdkReady={true} blocked={prepared?.allowed!==true} pin={pin} onPinChange={setPin} onConnect={()=>{window.syntheticMetaRequests=(window.syntheticMetaRequests||0)+1;}}/>
...
let stored = null, failNext = false, mode='normal';
...
const responseBody=()=>({ ...scope,companyName:'Constructora de ensayo',profile:tenantWorkspaceFromMetadata(stored?{whatsappWorkspace:stored}:{}),projects,projectsTruncated:false,automationActivated:false });
const server=createServer((req,res)=>{
  const path=new URL(req.url,'http://local').pathname;
  if(path==='/api/integrations/whatsapp/workspace'){
    assert.equal(req.headers['x-obrasaas-organization'],'company-a');assert.equal(req.headers['x-obrasaas-project'],'project-a');
    res.setHeader('Content-Type','application/json');res.setHeader('Cache-Control','no-store');
    if(req.method==='GET'){res.end(JSON.stringify(mode==='wrong-context'?{...responseBody(),organizationId:'foreign'}:responseBody()));return;}
    let text='';req.on('data',chunk=>text+=chunk);req.on('end',()=>{
      ...
      const body=responseBody();res.end(JSON.stringify({...body,unchanged:replay,authorization:workspaceAuthorizationState(body.profile,scope.projectId)}));
    });return;
  }
...
try{
  ...
  await expect(setup.getByText(/Preparación guardada para tu empresa/)).toBeVisible();
  ...
  await expect(setup.getByText(/Preparación guardada para tu empresa/)).toBeVisible();assert.deepEqual(requests.at(-1),requests.at(-2));assert.equal(stored.revision,3);
  ...
  await setup.getByRole('button',{name:'Editar preparación',exact:true}).click();
  await setup.getByLabel('Primera obra',{exact:true}).selectOption('project-b');await setup.getByRole('checkbox',{name:/Prepararé una cuenta/}).check();
  await setup.getByRole('button',{name:'Guardar preparación',exact:true}).click();
  await expect(setup.getByText(/Abrí Integraciones en la primera obra/)).toBeVisible();
  await expect(page.getByRole('button',{name:'Continuar en Meta',exact:true})).toBeDisabled();
  await page.reload();await expect(setup.getByRole('region',{name:'Configuración guardada del asistente'})).toBeVisible();
  await setup.getByRole('button',{name:'Editar preparación',exact:true}).click();
  await expect(setup.getByLabel('Nombre del asistente',{exact:true})).toHaveValue('Nuevo nombre no guardado');
  await expect(setup.getByLabel('Primera obra',{exact:true})).toHaveValue('project-b');
  await expect(page.getByRole('button',{name:'Conectar WhatsApp',exact:true})).toBeDisabled();
