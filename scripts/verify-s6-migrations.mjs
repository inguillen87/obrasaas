...
try {
  for (const migration of ['20260724170000_extra_work_requests', '20260724180000_replan_scenarios']) {
  ...
  const tables = await client.query('SELECT "tablename" FROM "pg_tables" WHERE "schemaname" = current_schema() AND "tablename" = ANY($1::text[])', [['ExtraWorkRequest', 'ReplanScenario']]);
  const found = new Set(tables.rows.map((row) => row.tablename));
  for (const table of ['ExtraWorkRequest', 'ReplanScenario']) if (!found.has(table)) throw new Error(`Missing table ${table}.`);
  ...
  console.log('Verified S6 migrations: extra work and isolated replan scenarios.');
