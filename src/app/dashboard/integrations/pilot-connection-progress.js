  ...
  return <section className={styles.panel} style={{ '--connect-bg': tokens.colors.bg.secondary, '--connect-border': tokens.colors.border.default, '--connect-accent': tokens.colors.accent.primary, '--connect-text': tokens.colors.text.primary, '--connect-muted': tokens.colors.text.secondary }} aria-label="Resultado real de las conexiones piloto">
    ...
    {progress.channels.map((channel, index) => <article key={index} className={styles.prepare}>
      ...
      <ol className={styles.steps}>
        ...
        <li><b>2</b><div><strong>{channel.received === null ? 'Recepción sin verificar' : channel.received + ' mensajes recibidos en el backend'}</strong><span>{channel.awaitingParticipant > 0 ? channel.awaitingParticipant + ' mensajes esperan vinculación del remitente a un participante autorizado. No se habilitan acciones de obra automáticamente.' : 'El registro de entrada se comprueba por separado del permiso del participante.'}</span></div></li>
      ...
      {channel.awaitingParticipant > 0 && <p className={styles.notice}>Siguiente paso: revisar el remitente dentro de esta empresa y autorizar su participación. No se lo incorpora a nómina ni se le asigna un rol por haber escrito al número.</p>}
