...
import { validPilotProofRecovery, pilotProofCanSend, pilotProofRecoveryLabel } from './pilot-proof-recovery';
...
export default function PilotChannelProofPanel({organizationId,projectId,targets}) {
 ...
 const [elapsed,setElapsed]=useState(0),[errorCode,setErrorCode]=useState(''),[mustRecheck,setMustRecheck]=useState(false);
 useEffect(()=>{if(!snapshot?.recovery?.credentialExpiresAt)return;const start=performance.now();const update=()=>setElapsed(performance.now()-start);const timer=setInterval(update,1000);return()=>clearInterval(timer);},[snapshot]);
 const canSend=pilotProofCanSend(snapshot,elapsed)&&!mustRecheck;
 ...
 function select(value){if(lock.current||uncertain)return;setTarget(value);setSnapshot(null);setReceipt(null);setError('');setConfirmed(false);setErrorCode('');setMustRecheck(false);attempt.current=null;}
 async function run(write=false){
  if(lock.current||!target||(!write&&uncertain)||write&&!confirmed&&!uncertain)return;
  if(write&&!attempt.current){if(!canSend)return;attempt.current={projectId:target,connectionId:snapshot.connectionId,inboundId:snapshot.inbound.id,confirmSend:true};}
  lock.current=true;setBusy(true);setError('');setErrorCode('');if(!write){setSnapshot(null);setReceipt(null);setConfirmed(false);}const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),55000);
  try{
   ...
   if(!response.ok){const failure=new Error(data?.error||'Comprobación no confirmada.');failure.status=response.status;failure.code=data?.code;throw failure;}
   ...
   else{if(!validPilotProofRecovery(data.recovery)||!data.connectionId||data.inbound&&(!data.inbound.id||!/^\d{4}$/.test(data.inbound.contactLast4)))throw new Error('Estado del canal incompleto.');setElapsed(0);setSnapshot(data);setReceipt(data.reply);setConfirmed(false);setMustRecheck(false);}
  }catch(failure){if(write){setUncertain(!failure.status||failure.status>=500);if(failure.status&&failure.status<500){attempt.current=null;setConfirmed(false);setMustRecheck(true);}}else if([401,403,404,409].includes(failure.status)){setSnapshot(null);setReceipt(null);}setErrorCode(failure.code||'');setError(failure.name==='AbortError'?'La respuesta demoró. Un envío podría haberse realizado; verificá el mismo intento.':failure.message);}
 ...
 return <section className={styles.panel} style={css} aria-labelledby="pilot-proof-heading">
  ...
  {snapshot?.recovery && <section className={styles.pilotRecovery} aria-label="Autorización actual del canal" role="status">
   <strong>{snapshot.recovery.blocker?.title || (canSend ? 'Autorización comprobada para este intento' : 'Comprobá nuevamente antes de enviar')}</strong>
   <p>{snapshot.recovery.blocker?.message || 'El servidor vuelve a validar el canal y la ventana de respuesta antes de enviar.'}</p>
   {snapshot.recovery.credentialExpiresAt && <p>Vencimiento registrado: {new Date(snapshot.recovery.credentialExpiresAt).toLocaleString('es-AR')}.</p>}
   {snapshot.recovery.blocker && <a href="#pilot-import-title">{pilotProofRecoveryLabel(snapshot.recovery.blocker.code)}</a>}
   <p>Renovar conserva la empresa, la obra y el número existentes. Esta consulta no renueva credenciales ni envía mensajes.</p>
  </section>}
  ...
  {snapshot?.inbound&&!receipt&&<div className={styles.pilotForm}>
   <label className={styles.pilotConsent}><input type="checkbox" disabled={busy||uncertain||!canSend} checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/>Confirmo enviar una única respuesta técnica al remitente terminado en {snapshot.inbound.contactLast4}, dentro de esta obra piloto.</label>
   <button type="button" disabled={busy||(!uncertain&&(!confirmed||!canSend))} onClick={()=>run(true)}>{uncertain?'Verificar el mismo envío':'Enviar respuesta de prueba'}</button>
   {snapshot.inbound.windowOpen===false&&<p>La ventana de respuesta no está abierta. El contacto debe enviar un mensaje nuevo; no se envían plantillas automáticamente.</p>}
  </div>}
  ...
  {errorCode&&<p className={styles.scope}><a href="#pilot-import-title">{pilotProofRecoveryLabel(errorCode)}</a> · Después, comprobá de nuevo la recepción antes de autorizar otro envío.</p>}
