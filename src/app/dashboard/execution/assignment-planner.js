...
export default function AssignmentPlanner({organizationId,projectId,tasks,focusedTask,onClose,onSaved}) {
  ...
  function change(key,value){setDraft(previous=>({...previous,[key]:value,...(key==='ownerKind'?{ownerId:''}:{})}));setConsent(false);setReview(null);}
  function refreshSource(){
    ...
    setSource(null);setReview(null);setConsent(false);setError('');setPhase('loading');setReviewEpoch(value=>value+1);setQueryEpoch(value=>value+1);
  }
