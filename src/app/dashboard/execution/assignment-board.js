...
export default function AssignmentBoard({assignments,tasks,workers,teams,permissions,organizationId,projectId,focusedTask,onChanged}){
  ...
  return <section id="task-assignments" className={styles.board} style={style} aria-label="Planificación y seguimiento de asignaciones">
    ...
    {canPlan&&!hasTask&&<p className={styles.hint}>No hay actividades asignables en este contexto. Las fases y los hitos no se asignan desde este formulario; revisÃ¡ las actividades del cronograma.</p>}
    <div className={styles.summary}><strong>{active}</strong><span>Planificadas o en curso en este listado</span><small>{assignments.length} {assignments.length===1?'asignaciÃ³n registrada':'asignaciones registradas'} · finalizar no certifica avance</small></div>
