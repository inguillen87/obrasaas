...
import CrewRosterPanel from './crew-roster-panel';
...
export default function ExecutionClient({ initialData, workers, tasks, permissions, organizationId, projectId, focusedBlockerId = null, focusedTask = null }) {
  ...
  return <section className={styles.content}>
    ...
    <div className={styles.grid}>
      <section className={styles.panel}><div className={styles.heading}><div><span className={styles.kicker}>Estructura operativa</span><h2>Cuadrillas</h2></div></div><CrewRosterPanel teams={data.teams} organizationId={organizationId} projectId={projectId} canManage={permissions.canManage}/>{permissions.canManage && <form className={styles.form} onSubmit={createTeam}><input value={teamName} onChange={(event) => setTeamName(event.target.value)} aria-label="Nombre de la cuadrilla" placeholder="Nombre de la cuadrilla" maxLength={160} /><button disabled={busy} type="submit">Crear equipo</button></form>}</section>
