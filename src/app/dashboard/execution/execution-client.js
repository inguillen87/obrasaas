...
export default function ExecutionClient({ initialData, workers, tasks, permissions }) {
  ...
  async function createBlocker(event) {
    ...
    setBusy(true); try { const result = await api({ method: 'POST', body: { kind: 'BLOCKER', title: blockerTitle, taskId: blockerTask || undefined, severity: 'MEDIUM' } }); setData((current) => ({ ...current, blockers: [result.blocker, ...current.blockers] })); setBlockerTitle(''); setBlockerTask(''); setNotice('Blocker abierto con owner pendiente de asignación.'); } catch (error) { setNotice(error.message); } finally { setBusy(false); }
  }
