...
import TaskRestrictionSummary, { restrictionsVerified } from './task-restriction-summary';
...
export default function ScheduleFieldPanel({ organizationId, projectId, onSnapshot }) {
  ...
  const allVerified = Boolean(snapshot) && rows.every(row => restrictionsVerified(row.restrictions));
  const activeRestrictions = rows.reduce((sum, row) => sum + (restrictionsVerified(row.restrictions) ? row.restrictions.active : 0), 0);
  const visibleRows = onlyRestricted ? rows.filter(row => restrictionsVerified(row.restrictions) && row.restrictions.active > 0) : rows;
  const guard = event => { if (!requestWorkspaceNavigation('route')) event.preventDefault(); };
  ...
  return <section id="field-schedule-status" className={styles.panel} style={{ '--field-accent': tokens.colors.accent.primary }} aria-label="Evidencia y avance en el cronograma">
    ...
    {taskId && <Link onNavigate={guard} prefetch={false} href="/dashboard?tab=sec-gantt" className={styles.back}>Ver todas las tareas</Link>}
    ...
    <div className={styles.restrictionFilter}><div><strong>{allVerified ? activeRestrictions : '—'} restricciones activas</strong><span>En las tareas de esta página, no en toda la obra.</span></div>
      <label><input type="checkbox" checked={onlyRestricted} disabled={!allVerified} onChange={event => setOnlyRestricted(event.target.checked)} />Mostrar sólo tarjetas con restricciones</label></div>
    {onlyRestricted && allVerified && visibleRows.length === 0 && <p className={styles.empty}>No hay restricciones activas registradas en estas tareas. El Gantt conserva todas sus barras.</p>}
    <div className={styles.tasks}>{visibleRows.map(task => <article key={task.id} className={styles.task}>
      ...
      {task.assignments && <p className={styles.assignmentHint}>{task.assignments.active} asignaciones en curso · {task.assignments.planned} planificadas. Son registros de asignación, no cantidad de trabajadores ni avance.</p>}
    ...
    {snapshot?.unassignedParts > 0 && <p className={styles.warning}>{snapshot.unassignedParts} partes de la obra todavía no están vinculados a una tarea. No se atribuyen automáticamente a ninguna barra. <Link onNavigate={guard} prefetch={false} href="/dashboard/progress?unassigned=1" className={styles.back}>Revisar partes sin tarea</Link></p>}
