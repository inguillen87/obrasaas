...
export default function PayablesClient({ initialInvoices, suppliers, purchaseOrders, projectName }) {
  const [rows, setRows] = useState(initialInvoices);
  const [notice, setNotice] = useState(null);
  const [busy, setBusy] = useState(false);
  const [file, setFile] = useState(null);
  const [form, setForm] = useState({ supplierId: suppliers[0]?.id || '', purchaseOrderId: '', invoiceNumber: '', amount: '', currency: 'ARS', dueAt: '' });
