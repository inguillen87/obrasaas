...
import {
  ...
  goodsReceiptInspectionErrorResponse,
  listGoodsReceiptInspections,
} from '@/lib/goods-receipt-inspections';
...
function known(request, error) {
  ...
  const domain = projectWritePolicyErrorResponse(error)
    || goodsReceiptInspectionErrorResponse(error);
...
export async function POST(request) {
  try {
    ...
    // Receipt inspection currently shares execution permissions. A dedicated
    // inspector/receiver segregation remains a release-policy follow-up.
    ...
    const result = await createGoodsReceiptInspection(getPrisma(), {
      scope: {
    ...
    return known(request, error) || respond(request, Response.json({
      error: 'No se pudo registrar la inspección de recepción.',
      code: 'GOODS_RECEIPT_INSPECTION_WRITE_FAILED',
    }, { status: 500 }));
  }
}
...
export async function GET(request) {
  try {
    ...
    const result = await listGoodsReceiptInspections(getPrisma(), {
      organizationId: access.organization.id,
      projectId: access.project.id,
      purchaseOrderId: scheduleQueryValue(searchParams, 'purchaseOrderId'),
      goodsReceiptId: scheduleQueryValue(searchParams, 'goodsReceiptId'),
    ...
    return known(request, error) || respond(request, Response.json({
      error: 'No se pudieron cargar las inspecciones de recepción.',
      code: 'GOODS_RECEIPT_INSPECTION_READ_FAILED',
    }, { status: 500 }));
  }
}
