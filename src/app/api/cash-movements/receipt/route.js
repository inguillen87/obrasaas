...
function errorResponse(error) {
  ...
  return projectWritePolicyErrorResponse(error)
    ...
    || json({ error: 'No se pudo procesar el comprobante privado.', code: 'CASH_RECEIPT_UPLOAD_FAILED' }, { status: 500 });
}
...
export async function POST(request) {
  try {
    ...
    if (!(file instanceof File) || file.size < 1 || file.size > MAX_PROTECTED_UPLOAD_BYTES) {
      return json({ error: protectedUploadFileSizeMessage('El comprobante'), code: 'CASH_RECEIPT_SIZE_INVALID' }, { status: 400 });
    }
    if (!ALLOWED.has(file.type)) {
      return json({ error: 'Tipo de comprobante no permitido.', code: 'CASH_RECEIPT_TYPE_INVALID' }, { status: 400 });
    }
    const bytes = Buffer.from(await file.arrayBuffer());
    if (!receiptBytesMatchMime(bytes, file.type)) {
      return json({ error: 'El contenido no coincide con el tipo de comprobante declarado.', code: 'CASH_RECEIPT_CONTENT_INVALID' }, { status: 400 });
    }
