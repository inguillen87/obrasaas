...
import { progressJournalErrorResponse, reviewProgressRecord } from '@/lib/progress-journal';
...
export async function PATCH(request, { params }) { try { const access = await getPlatformAccess(); requireTenantPermission(access, 'org:execution:manage', { subscriptionMode: 'write' }); const input = await readJsonRequest(request, { maxBytes: 16 * 1024 }); const { recordId } = await params; return Response.json(await reviewProgressRecord(getPrisma(), { scope: { organizationId: access.organization.id, projectId: access.project.id }, actorId: access.databaseUserId, id: recordId, kind: input.kind, status: input.status, expected: input.expectedRevision, reviewNote: input.reviewNote })); } catch (error) { return known(error) || Response.json({ error: 'No se pudo revisar el registro.', code: 'PROGRESS_REVIEW_FAILED' }, { status: 500 }); } }
