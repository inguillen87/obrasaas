...
export async function GET(request) {
  try {
    ...
    return Response.json(await listWorkerDocuments(getPrisma(), { projectId: access.project.id, workerId: params.get('workerId') || undefined, status: params.get('status') || undefined }), { headers: { 'Cache-Control': 'private, no-store' } });
  } catch (error) {
