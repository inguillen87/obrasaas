...
import { prepareTaskAssignment, planTaskAssignment, getTaskAssignment, decideTaskAssignment } from '@/lib/task-assignments';
...
export function createTaskAssignmentHandlers({resolveAccess=getPlatformAccess,authorize=requireTenantPermission,database=getPrisma,
  prepare=prepareTaskAssignment,plan=planTaskAssignment,read=getTaskAssignment,decide=decideTaskAssignment,parse=request=>readJsonRequest(request,{maxBytes:8192})}={}) {
  async function context(request,write,withTask=false){
    ...
    if(write&&((request.headers.get('origin')&&request.headers.get('origin')!==url.origin)||request.headers.get('sec-fetch-site')==='cross-site'))throw new TaskAssignmentError('Origen no autorizado.','ASSIGNMENT_ORIGIN',403);
