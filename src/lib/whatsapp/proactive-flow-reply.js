...
// No connection/token lookup or business write. A caller supplies only the
// OUTBOUND message id; the INBOUND id is derived from the scoped consumed session.
export async function readProactiveFlowReply({ prisma, access, conversationId, messageId, clock = () => new Date() }) {
  ...
  return prisma.$transaction(async tx => {
    ...
    const result = (state, reply = null) => {
      const payload = { context: scope, sourceMessageId: messageId, observedAt, state, reply };
    ...
    if (!uuid(meta.flowSessionId) || !reference(source.externalId)) return result('unavailable');
    const session = await tx.whatsAppFlowSession.findFirst({ where: { id: meta.flowSessionId, organizationId: scope.organizationId, projectId: scope.projectId,
      ...
      select: { id: true, workerId: true, createdAt: true, consumedAt: true, consumedExternalId: true, sentAt: true, providerMessageId: true, deliveryRejectedAt: true } });
    if (!session) return result('unavailable');
    if (!session.consumedAt && !session.consumedExternalId) return result('not_recorded');
    ...
    if (!processedAt || !issuedAt || !sentAt || processedAt < issuedAt || sentAt < issuedAt || processedAt < sentAt
      ...
      || !reference(source.providerMessageId) || source.providerMessageId !== session.providerMessageId) return result('unavailable');
    ...
    if (!inbound) return result('unavailable');
    ...
    if (inboundMeta.provider !== 'meta' || inboundMeta.authorized !== true || inboundMeta.quarantined === true
      ...
      || !recordedAt || recordedAt < issuedAt || recordedAt > observedAt) return result('unavailable');
    return result('available', { ...presentWhatsAppReplyExcerpt(inbound), processedAt });
