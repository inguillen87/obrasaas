...
El blueprint, el Data Endpoint, el panel CRM y el reconciliador DB-only están implementados localmente. H4 consta de cuatro migraciones coordinadas: `20260729130000_worker_payment_privacy_choices`, `20260729131000_worker_payment_privacy_choices_validate`, `20260729132000_worker_payment_flow_sessions` y `20260729133000_worker_payment_flow_reconciliation`; aplican juntas en PGlite local. El Flow permanece fuera del catálogo proactivo genérico por diseño, pero ya tiene un caller inbound determinístico: una intención textual `PAYMENT_DESTINATION` sólo lo habilita para resolución `CANONICAL`, persona y canal verificados, obra activa y referencia publicada. La sesión genérica y su companion se emiten dentro de la misma transacción autoritativa que persiste outcome y mensajes; el POST a Meta ocurre después del commit mediante el journal anti-duplicado, por lo que no existe atomicidad DB–Meta. Si el texto contiene un CBU/CVU de 22 dígitos o una declaración explícita de CBU/CVU/alias, se reemplaza antes de persistir la cola. Antes de aplicar las migraciones sobre datos existentes debe coordinarse el corte de re-atestación legacy. Siguen pendientes PostgreSQL real/Neon Preview, build y smoke H4 en Vercel Preview, publicación de plantilla/Flow en Meta y E2E real. No hubo migración ni despliegue H4 en Production; esta sección acredita contrato local, no disponibilidad por WhatsApp.
