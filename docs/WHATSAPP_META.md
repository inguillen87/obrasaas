...
En la cuenta de Meta revisada para ObraSaaS ya están presentes la app dedicada y el caso de uso de WhatsApp. Meta también asignó un número de prueba, se verificó un celular propio como destinatario y el panel generó una credencial temporal. En una ejecución histórica Meta aceptó una solicitud outbound de plantilla. En este corte, el test oficial del campo `messages v25.0` llegó al Preview con HTTP 200; el payload ficticio fue descartado internamente por no corresponder a una conexión tenant, como exige el aislamiento. Esta documentación omite deliberadamente App IDs, WABA IDs, Phone Number IDs, teléfonos y valores secretos.

La evidencia actual prueba aceptación outbound histórica y recepción de un test oficial firmado en el endpoint correcto; no prueba entrega al celular, inbound real, eventos de estado correlacionados, Flows, aislamiento sobre un tenant conectado ni un circuito bidireccional end-to-end. Un piloto aislado puede usar un token temporal recién generado, persistido cifrado por conexión desde Integraciones y con vencimiento operativo explícito. Una liberación sostenida exige un System User token con permisos mínimos y rotación gestionada; ninguna credencial temporal es apta para Production. La app continúa sin publicar y el negocio no verificado, por lo que la verificación empresarial/revisión aplicable y publicación son gates externos previos al inbound real.
...
El backend implementa `INIT`, `BACK`, `data_exchange`, `ping` y el acuse de errores de salud. Los datos de organización, proyecto, trabajador, Flow y pantalla no se aceptan como autoridad desde el cliente: se vuelven a resolver desde la conexión y la sesión persistida. Para los Flows operativos genéricos, el Data Endpoint entrega contexto y opciones y la mutación definitiva sigue entrando por el evento terminal `nfm_reply`. El Flow especializado de destino de cobro es la excepción deliberada: persiste su consentimiento y destino cifrado durante el `data_exchange`, y devuelve únicamente una referencia opaca y estado neutral.

### Flow especializado de destino de cobro

`worker-payment-destination` usa una sesión companion 1:1 con la sesión genérica y fija organización, obra, conexión, operario, persona, identidad WhatsApp verificada, aviso y vencimiento. `INIT` sólo registra que el aviso exacto fue presentado después de un intento de entrega válido; no prueba lectura o comprensión. La reserva guarda un HMAC SHA-256 del formulario, nunca CBU, CVU o alias crudo. El HMAC usa el keyring versionado `WORKER_PAYMENT_FLOW_SESSION_HMAC_KEYRING_JSON` y la clave indicada por `WORKER_PAYMENT_FLOW_SESSION_HMAC_ACTIVE_KEY_ID`. Una clave se retira sólo cuando el helper con reloj de base informa cero: toda fila bloquea hasta `expiresAt + 24h` y `PROCESSING` bloquea sin límite. La secuencia es desplegar la clave nueva conservando la anterior, drenar instancias/in-flight, ejecutar el gate y recién entonces retirar la anterior; el modo single-key usa el ID exacto `legacy-v1`. `WORKER_PAYMENT_FLOW_SESSION_SECRET` queda sólo como compatibilidad de transición. El camino normal es `OPEN → PROCESSING → SUCCEEDED|UNCERTAIN`; una fila `UNCERTAIN` nunca vuelve a `PROCESSING` ni reintenta la mutación, y sólo puede avanzar a `SUCCEEDED` si el reconciliador prueba en base un destino previamente confirmado con procedencia exacta.

`OPEN → PROCESSING` exige más de un minuto restante según `statement_timestamp()`. El cron autenticado mueve un `PROCESSING` vencido y estable sólo a `UNCERTAIN`, con auditoría en la misma transacción y sin invocar el bridge. Después, el reconciliador DB-only —también invocado de forma tenant-scoped por un retry exacto— sólo enlaza un destino ya persistido cuando coinciden reserva, HMAC, tipo/fingerprint, claves de operación, canal, aviso y consentimiento; el trigger sella `submissionReconciledAt` y `OPERATION_PROVENANCE_V1` con reloj de PostgreSQL. No recibe el formulario ni llama a bridge, proveedor, pago, WhatsApp o IA. Si la sesión queda `SUCCEEDED`, un `data_exchange` exacto puede reconstruir únicamente el comprobante allowlisted durante 24 horas tras volver a autenticar token/scope, formulario por HMAC, entrega, consumo y canal no revocado. Si no existe resultado o la procedencia difiere, permanece `UNCERTAIN` para resolución auditada. Los journals enlazados a `PROCESSING`/`UNCERTAIN` se conservan de forma deliberada mientras esos estados siguen abiertos; esto continúa siendo deuda explícita de retención/DSAR.

El blueprint, el Data Endpoint, el panel CRM, el reconciliador DB-only y la constancia privada por opt-in están implementados localmente. H4 consta de cinco migraciones coordinadas: `20260729130000_worker_payment_privacy_choices`, `20260729131000_worker_payment_privacy_choices_validate`, `20260729132000_worker_payment_flow_sessions`, `20260729133000_worker_payment_flow_reconciliation` y `20260729134000_worker_payment_private_receipts`; las cinco (`13000-13400`) aplican juntas en PGlite local. El Flow permanece fuera del catálogo proactivo genérico por diseño, pero ya tiene un caller inbound determinístico: una intención textual `PAYMENT_DESTINATION` sólo lo habilita para resolución `CANONICAL`, persona y canal verificados, obra activa y referencia publicada. La sesión genérica y su companion se emiten dentro de la misma transacción autoritativa que persiste outcome y mensajes; el POST a Meta ocurre después del commit mediante el journal anti-duplicado, por lo que no existe atomicidad DB–Meta. Si el texto contiene un CBU/CVU de 22 dígitos o una declaración explícita de CBU/CVU/alias, se reemplaza antes de persistir la cola. Un opt-in terminal exacto emite únicamente un descriptor durable no secreto; el enlace de 15 minutos se reconstruye después del claim de envío, la webview y el PDF no muestran el destino completo y la constancia no acredita titularidad ni pago. Antes de aplicar las migraciones sobre datos existentes debe coordinarse el corte de re-atestación legacy. Siguen pendientes PostgreSQL real/Neon Preview, build y smoke H4 en Vercel Preview, publicación de plantilla/Flow en Meta y E2E real. No hubo migración ni despliegue H4 en Production; esta sección acredita contrato local, no disponibilidad por WhatsApp.
...
La implementación limita atómicamente cada conexión a 600 solicitudes nuevas por minuto y usa un lease de 12 segundos para evitar procesadores concurrentes. Un terminal sin respuesta cifrada nunca fue observable y queda inmediatamente reclamable bajo un lease nuevo; no funciona como caché negativa de 24 horas. En particular, una conciliación temporalmente no disponible responde `503` sin body/ciphertext y `Retry-After: 1`, de modo que el siguiente request exacto vuelve a consultar el estado durable. Cuando ya existe una respuesta cifrada confirmada, el hash y su ciphertext sí pasan a ser un tombstone criptográfico: se reproducen byte a byte y no se eliminan sólo por `expiresAt` mientras la versión RSA correspondiente siga `ACTIVE` o `RETIRING`. Esto evita reutilizar el nonce AES-GCM obligatorio del protocolo con otro plaintext. La función tiene un máximo operativo de 10 segundos. No llama a OpenAI, Cloudinary ni otros servicios ajenos al contrato del Flow durante ese camino síncrono.

El GC usa `garbageCollectWhatsAppFlowEndpointRequests` en lotes acotados y por endpoint. Puede eliminar registros terminales sin ciphertext y leases vencidos sólo después de cumplir la retención de 24 horas; esa retención para GC no impide su reclamación inmediata. Además, no elimina un journal ligado a una sesión de cobro todavía `PROCESSING` o `UNCERTAIN`. Un registro que ya contiene ciphertext sólo es elegible cuando su versión RSA está `REVOKED`, o `RETIRING` ya vencida, y pasó además una gracia de 10 minutos para drenar handlers que hubieran cargado el keyring antes del retiro. Las versiones `ACTIVE`, `STAGED`, `RETIRING` vigentes, las versiones desconocidas y los tombstones sin `keyVersion` se conservan. `vercel.json` agenda `/api/cron/webhooks` cada minuto en Production, una frecuencia que requiere Vercel Pro; Preview debe invocarlo manualmente. La ruta ejecuta el GC en modo best-effort, con un máximo de 2 endpoints y 250 filas por endpoint; `hasMore=true` deja el remanente para la ejecución siguiente sin agregar latencia al Data Endpoint. Vercel puede entregar una corrida más de una vez, por lo que los leases y la idempotencia siguen siendo parte del contrato operativo.
...
Esta lista describe el código y sus pruebas de contrato. Como evidencia externa separada, Meta ya asignó el número de prueba, verificó un destinatario propio, aceptó históricamente una solicitud outbound y entregó correctamente el test oficial firmado `messages v25.0` al endpoint Preview. Eso no afirma entrega ni que un WABA/tenant real haya completado el circuito. Vercel Preview está `Ready`, el alias estable responde y `META_APP_SECRET` está limitado a la rama. No hubo deployment ni migración de aplicación en Production durante este corte; subsiste el incidente de posible sincronización de membresía por el webhook compartido de Clerk development ([evidencia](./evidence/2026-07-28-preview-c91cee0.md)). Todavía faltan la publicación de la app, tenant/import cifrado, smoke UI/runtime, observar el cron H3.1 y recorrer Meta end-to-end.
...
2. verificar los secretos branch-scoped ya presentes —incluido `META_APP_SECRET`— y completar cualquier faltante entre `META_VERIFY_TOKEN`, `NEXT_PUBLIC_META_APP_ID`, `NEXT_PUBLIC_META_EMBEDDED_SIGNUP_CONFIG_ID`, `WHATSAPP_CREDENTIALS_ENCRYPTION_KEY`, `WHATSAPP_FLOW_TOKEN_SECRET`, `WORKER_ONBOARDING_FLOW_TOKEN_SECRET`, `WHATSAPP_PREVIEW_ALLOWED_PUBLIC_ORIGINS` y el registro KEK, sin exponer sus valores; el token de cada tenant debe persistirse sólo cifrado mediante el flujo previsto;
...
4. completar la verificación empresarial/revisión aplicable y publicar la app; el test oficial de panel ya alcanzó el webhook, pero luego hay que persistir al menos un inbound real y eventos de estado correlacionados con un outbound aceptado;
