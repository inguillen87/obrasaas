...
**Corte de evidencia:** 12 de agosto de 2026

**Rama de trabajo:** `codex/platform-ux-foundation`

**Estado:** especificación funcional v1.0 de la clienta integrada y contrastada
contra el repositorio. S9.3-CONTRACT está cerrado técnicamente con PostgreSQL,
Quality, Browser público, E2E autenticado Development y Preview exact-SHA
verdes sobre el mismo commit.
La evidencia se separa entre código/pruebas locales, migración en Preview
aislado, journey UI y E2E externo. No hubo deployment ni migración de aplicación
en Production durante este corte; subsiste el incidente documentado de una
posible sincronización de membresía causada por el webhook compartido de Clerk
development ([evidencia](./evidence/2026-07-28-preview-c91cee0.md)).
...
| --- | --- |
| Commit/CI reproducible | El commit `21ce752` cerró CI [31558109020](https://github.com/inguillen87/obrasaas/actions/runs/31558109020) 4/4 sobre el SHA completo. PostgreSQL 17 aplicó 124 migraciones, aprobó S9.3 con carreras/cleanup y flag descartable `1`, status y drift cero; Quality aprobó `2446/2446`, lint, auditoría de producción sin vulnerabilidades y build Next.js 16.2.11 de 93 páginas; Browser público quedó verde. El E2E autenticado verificó fixtures Clerk Development sin drift y cerró S9.2 2/2 más S9.3 2/2 sobre PostgreSQL descartable. El [corte S9.3](./evidence/2026-08-12-preview-21ce752.md) fija jobs, journey, un warning P2 de compatibilidad futura con `pg@9` y los límites. El [corte S9.2-E2E](./evidence/2026-08-11-preview-dcb44b9.md) y los gates previos permanecen como evidencia histórica independiente |
| Último Preview reproducible | El deployment inmutable `dpl_2cmWjw8AtEqLquafAZBXWbHSeHRW` del commit `21ce752` quedó `READY`: Neon detectó 124 migraciones sin pendientes, S9.3 pasó rollback-only con flag descartable final `0` y el build cerró 93/93. Los cuatro smokes públicos read-only dieron `200`; en la ventana observada hubo cero `error`, `fatal` o `5xx`. No hubo sesión, POST S9.3, alias ni Production en Preview ([evidencia](./evidence/2026-08-12-preview-21ce752.md)) |
...
ObraSaaS ya es una base SaaS real; no es sólo una maqueta. Tiene tenancy B2B,
roles operativos, alcance por obra, auditoría, proyectos, Gantt, WhatsApp
persistente, Flows gobernados, aprobaciones humanas, evidencia privada,
asistencia y reportes PDF. Prisma, lint, build y la suite local permanecen como
gates de cada corte. Baseline/forecast, la medición técnica cuantitativa S9.1 y
el corte técnico quincenal reproducible S9.2-MED fueron verificados en Preview
aislado; S9.2 agregó un journey autenticado completo en CI Development
descartable. S9.3 ya implementa y cerró técnicamente la autoridad
contractual/SOV separada con PostgreSQL, Quality, Browser público, E2E
autenticado Development y Preview exact-SHA verdes. La UI dedicada de S9.1 permanece
pendiente. El puente local revisión visual → observación inmutable →
forecast/Gantt todavía requiere aplicar su nueva migración y recorrer el journey
...
| Medición técnica de avance | S9.1 y S9.2-MED verificados en CI PostgreSQL 17 y Preview; S9.2-E2E autenticado cerrado en Development descartable | El [ledger cuantitativo S9.1](./PROGRESS_MEASUREMENTS_S9_1.md) conserva unidad, base, cantidad ejecutada, evidencia aprobada, Decimal exacto y maker-checker. El [corte S9.2-MED](./PROGRESS_MEASUREMENT_CUTS_S9_2.md) sella todas las tareas canónicas de la obra/quincena como `MEASURED` o `MISSING`, con composición server-owned, versión, hashes, replay y doble CAS. Seis actores en dos tenants recorrieron maker-checker, roles negativos, cross-tenant, seal/replay/stale y lectura UI; ninguno de los artefactos modifica `Task.progress`, baseline, forecast, presupuesto, certificado ni pago | Completar el journey UI dedicado de S9.1. S10 sólo podrá consumir el par exacto corte técnico + versión contractual S9.3. S9.2 no es certificado ni instrucción de pago |
| Autoridad contractual/SOV | S9.3 cerrado técnicamente en CI Development y Preview exact-SHA | [S9.3-CONTRACT](./PROJECT_CONTRACT_AUTHORITY_S9_3.md) aporta head/versión/líneas/decisiones append-only, cobertura `VALUED`/`NO_CLAIM`, dinero exacto, retención, maker-checker, tres autoridades distintas, API/UI privada y controles tenant/obra. El [corte `21ce752`](./evidence/2026-08-12-preview-21ce752.md) acredita CI 4/4, 124 migraciones sin drift, E2E autenticado S9.3 2/2 y Preview `READY` | S10-CERT Fase 1 ya consume el par exacto S9.2/S9.3 sin reutilizar presupuesto, AP, PDF ni estado de pago; Production mantiene gate explícito separado |
| Certificación contractual de avance | S10-CERT Fase 1 backend cerrado técnicamente; UI S10 pendiente | El [contrato S10-CERT](./PROJECT_CERTIFICATES_S10_CERT.md) aporta book/head/versión/líneas/deducciones/decisiones/recibos append-only, dinero exacto, maker-checker, replay, CAS y locks compartidos. El [corte `8a3f446`](./evidence/2026-08-12-preview-8a3f446.md) acredita CI 4/4, 126 migraciones sin drift, E2E autenticado S10-CERT 2/2 y Preview `READY` | Construir la UI S10 en un incremento separado y mantener S10-FIN, S10-PAYREF, PDF, AP y Production fuera de este cierre; no atribuir validez legal, contable o fiscal |
...
| CI y E2E | Parcial con gates S9.2/S9.3 autenticados reproducibles | El corte `21ce752` aprobó `2446/2446`, Prisma, lint, build 93/93, auditoría sin vulnerabilidades, PostgreSQL 17 con 124 migraciones/drift cero, Browser público y E2E autenticado. S9.2 y S9.3 cerraron 2/2 cada uno con actores Clerk Development, dos tenants y DB descartable; Preview repitió S9.3 rollback-only sin POST | Completar S9.1 UI, POST maker-checker de privacidad y journeys core/Meta/media/storage/visión. Los negativos tenant/obra de S9.2/S9.3 están cubiertos, no así una matriz global de todas las superficies; CI verde no equivale a operación real o Production |
...
| Gate | Evidencia de salida | Estado al 12/08/2026 |
| --- | --- | --- |
...
| Calidad automatizada | CI remota verde con instalación exacta, lint, suite completa, auditoría, build y smokes público/autenticado | `21ce752`: `2446/2446`, Prisma, lint, auditoría con cero vulnerabilidades, build 93/93, PostgreSQL 17 con 124 migraciones/drift cero, S9.1/S9.2/S9.3, S12.2C, PRO-05A/B.1, Browser público y S9.2/S9.3 autenticados 2/2 cada uno; CI GitHub `31558109020` terminó 4/4 verde |
| Journey operativo E2E | Admin crea obra/trabajador/tarea, aprueba propuesta y descarga reporte; roles restringidos fallan correctamente | Parcial: S9.2-E2E cubre maker-checker, roles sin sellado, dos tenants, negativos cross-tenant, seal/replay/stale y UI final; S9.3 cubre autoridad y SOV completa a través de roles exactos, replay y límites tenant/obra sobre Clerk Development/PostgreSQL descartable. Aún no cubre alta de trabajador, flujo core completo, descarga de reporte, proveedores externos ni POST de privacidad |
...
| Migraciones gobernadas | Postgres efímero, `migrate deploy`, estado, diff sin drift, smoke y rollback documentados | CI PostgreSQL 17 aprobó 124 migraciones y los gates S9.1/S9.2/S9.3, S12.2C y PRO-05A/B.1 en `21ce752`; cerró `migrate status` y drift cero. El job autenticado aplicó aparte las 124 migraciones a `obrasaas_e2e` descartable. Vercel/Neon Preview detectó las mismas 124 sin pendientes y aprobó S9.3 rollback-only con flag final `0`. Promoción atómica/drenaje y Production siguen pendientes; no admitir pods v1/v2 balanceados |
...
| S10 | Certificado, conformidad financiera, referencia externa, PDF contractual, deducciones y significado jurídico del certificado sobre la SOV S9.3 ya cerrada técnicamente |
...
| S9.3-CONTRACT | Autoridad contractual y SOV | [head/versión/líneas/decisiones propios](./PROJECT_CONTRACT_AUTHORITY_S9_3.md); todas las tareas `VALUED` o `NO_CLAIM`, moneda/escala, importe en minor units, retención bps, redondeo y autoridades versionadas | cerrado técnicamente en `21ce752`: maker-checker, tres membresías activas distintas, API/UI, CI 4/4 y Preview exact-SHA; sin reutilizar `BudgetVersion`, `SupplierInvoice`, certificado ni pago |
| S10 | Certificación en tres fases separadas | [S10-CERT Fase 1](./PROJECT_CERTIFICATES_S10_CERT.md) tiene backend/API privada cerrados técnicamente: consume cut S9.2 + contrato/autoridad S9.3 exactos; la UI S10, conformidad financiera, referencia externa de pago y PDF privado determinista siguen como incrementos separados | `VALUED + MISSING` bloquea absolutamente, sin conversión a `NO_CLAIM` ni cero; corrección crea versión; certificado/conformidad/referencia nunca crean `PAID` ni ejecutan o prueban pago |
...
Estado S9.1/S9.2-MED actualizado al 11 de agosto de 2026: el [ledger de mediciones](./PROGRESS_MEASUREMENTS_S9_1.md) y el [corte técnico quincenal](./PROGRESS_MEASUREMENT_CUTS_S9_2.md) quedaron verificados sobre 123 migraciones en PostgreSQL 17 y Vercel/Neon Preview en el [corte técnico `cc5aa21`](./evidence/2026-08-11-preview-cc5aa21.md). El [corte funcional `dcb44b9`](./evidence/2026-08-11-preview-dcb44b9.md) agregó CI 4/4 y un E2E autenticado con seis actores Development en dos tenants: maker-checker, roles negativos, cross-tenant, seal v1/v2, replay, `STALE` y lectura UI. Esa mutación ocurrió sólo sobre PostgreSQL descartable; Preview permaneció read-only. S9.2 conserva ausencias como `MISSING`, nunca como cantidad cero, y no crea importes, certificado, PDF contractual, conformidad financiera, estado de pago ni mutación de `Task.progress`. La certificación conserva su autoridad separada; el cierre backend S10-CERT se documenta abajo.

Estado S9.3-CONTRACT actualizado al 12 de agosto de 2026: el [contrato
canónico](./PROJECT_CONTRACT_AUTHORITY_S9_3.md) está implementado y cerrado
técnicamente como prerrequisito de S10. El [corte `21ce752`](./evidence/2026-08-12-preview-21ce752.md)
aprobó CI 4/4, 124 migraciones sin drift, Quality 2446/2446, Browser público,
E2E autenticado S9.3 2/2 y Preview exact-SHA `READY`. `BudgetVersion` continúa
siendo presupuesto interno y el [runbook histórico de cuentas por
pagar](./S10_PAYABLES_RUNBOOK.md) queda clasificado como legacy/AP; ninguno es
reutilizable como SOV, certificado, conformidad o referencia de pago. S10-CERT
Fase 1 ya está cerrado como backend; la UI S10, S10-FIN, S10-PAYREF, PDF y
Production permanecen pendientes.

Estado S10-CERT Fase 1 actualizado al 12 de agosto de 2026: el [corte
`8a3f446`](./evidence/2026-08-12-preview-8a3f446.md) cerró CI 4/4 con 2487/2487
pruebas, 126 migraciones sin pendientes ni drift, manifest de 12 carreras
committed, cleanup exacto y E2E autenticado S9.2/S9.3/S10-CERT 6/6 sobre Clerk
Development y PostgreSQL descartable. El Preview exacto quedó `READY` tras
aplicar las 126 migraciones y ejecutar S10 rollback-only; no hubo sesión
autenticada ni POST privado contra Preview. El cierre es exclusivamente
backend/API: no existe aún UI S10 y no acredita conformidad financiera,
referencia de pago, PDF, cuentas por pagar, `PAID`, Production ni validez legal,
contable o fiscal.
