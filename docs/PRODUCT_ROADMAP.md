...
| --- | --- |
| WIP local | El worktree contiene 116 migraciones. Se conserva la evidencia local de H4 `13000-13400` y PRO-05A ya documentada; la última migración, `20260802180000_task_material_requirements`, pertenece a S12.2B y sigue sólo local. Su dominio, API, UI y contratos tienen 27 pruebas focalizadas verdes. Esto no reemplaza PostgreSQL real/Neon Preview ni un journey autenticado |
| Último Preview reproducible | 115 migraciones y commit `4760a50` `Ready`; el corte acredita S12.2A, no S12.2B ([evidencia](./evidence/2026-08-02-preview-4760a50.md)) |
...
| Privacidad y derechos del titular | PRO-05A local, discovery-only y bloqueado | [Caso y manifiesto tenant-scoped](./DATA_SUBJECT_RIGHTS_FOUNDATION.md), atestación ADMIN que no suplanta identidad, catálogo v1 fijado, consultas read-only, HMAC sin identificadores fuente, replay exacto, rate limit durable y guardas DB append-only. `140000` aplica con las 108 migraciones previas y el verificador conductual pasa en PGlite; no hay ejecución destructiva ni prueba Neon/Vercel | Completar identidad/representación y decisiones PRO-05B, adapters PRO-05C, propagación y restore PRO-05D, matriz legal/entidad responsable y verificación externa antes de trabajadores reales |
...
| Materiales y stock | S12.2A en Preview; S12.2B local, no Preview | [Ledger on-hand](./INVENTORY_STOCK_LEDGER.md) con putaway/reversión y balance DB-owned; [BOM por tarea](./TASK_MATERIAL_REQUIREMENTS.md) local, versionada, append-only, con cantidades exactas y declaración explícita sin materiales | Aplicar/verificar S12.2B en Preview y construir S12.2C. `SupplierCommitment` es **PROMESA, NO RESERVA**; `AVAILABLE` exige reserva exacta suficiente |
...
[PRO-05A](./DATA_SUBJECT_RIGHTS_FOUNDATION.md) ya tiene una base local no destructiva: registro tenant-scoped, atestación administrativa separada de la identidad del solicitante, discovery `READ ONLY`, catálogo y manifiesto inmutables, blockers obligatorios y rate limit durable. Su migración `140000` aplica desde cero con las 108 anteriores y el verificador conductual pasa en PGlite; no se aplicó en Neon ni se validó en Vercel. PRO-05 continúa **abierto** porque esta fase sólo puede terminar bloqueada. PRO-05B debe incorporar identidad/representación, plazos, bases y holds; PRO-05C, acciones idempotentes por dominio; PRO-05D, terceros, backups y tombstone replay. Sin entidad legal/domicilio, matriz aprobada y esas pruebas externas no se habilitan datos de trabajadores reales.
