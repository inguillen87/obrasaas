...
| S10 | Certificación en tres fases separadas | [S10-CERT Fase 1](./PROJECT_CERTIFICATES_S10_CERT.md) tiene contrato de implementación congelado y backend pendiente: consume cut S9.2 + contrato/autoridad S9.3 exactos; luego siguen conformidad financiera y referencia externa de pago append-only; PDF privado determinista separado | `VALUED + MISSING` bloquea absolutamente, sin conversión a `NO_CLAIM` ni cero; corrección crea versión; certificado/conformidad/referencia nunca crean `PAID` ni ejecutan o prueban pago |
...
reutilizable como SOV, certificado, conformidad o referencia de pago. S10 y
