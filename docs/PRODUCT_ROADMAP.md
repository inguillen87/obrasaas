...
**Estado:** especificación funcional v1.0 de la clienta integrada y contrastada contra el repositorio. La evidencia se separa entre código/pruebas locales, migración en Preview aislado, journey UI y E2E externo; Production no fue modificada.
...
ObraSaaS ya es una base SaaS real; no es sólo una maqueta. Tiene tenancy B2B, roles operativos, alcance por obra, auditoría, proyectos, Gantt, WhatsApp persistente, Flows gobernados, aprobaciones humanas, evidencia privada, asistencia y reportes PDF. La suite local alcanza 1.369 pruebas unitarias y de integración liviana en este corte; lint y build también están verdes. Baseline/forecast y su migración fueron verificados en Preview aislado, aunque aún falta completar el journey UI final de esa operación.
...
| WhatsApp e Inbox | Confirmado por contrato; externo pendiente | Meta Cloud API directa como camino primario; app/caso de uso presentes; test number asignado, un celular propio verificado y solicitud outbound de plantilla aceptada con token temporal. Firma, idempotencia, leases, Inbox, Flows operativos, Flow pre-operario H3.1, media privada y vínculo foto→`ProgressEvidence` están cubiertos localmente | La aceptación outbound no prueba entrega ni E2E. Rotar el token temporal, instalar credenciales permanentes en Vercel y completar webhook firmado, inbound, estados, todos los Flows, storage y tenant real; Twilio no sustituye esos gates |
| IA visual y OCR | Contrato y migraciones en Preview aislado; activación API bloqueada | `VisualProgressAssessment`, opt-in tenant, lease recuperable, sanitización, rango/abstención y revisión CAS; OpenAI `gpt-5.6-sol` es candidato primario y HF/Z.ai challengers por contrato | Elegir una clave de piloto, probar foto Meta real y benchmark consentido antes de cualquier claim productivo; verificar DPA/ZDR o retención aceptada por proveedor y nunca certificar/pagar/reprogramar desde una foto |
...
| Identidad laboral y destino de cobro | Parcial; H3.1 local, Preview/Meta pendientes | H3.1 incluye Flow pre-operario, aviso fijado, acuse terminal, CRM/readiness y purga transitoria; cobro incluye validación CUIL/CBU/CVU/alias, cifrado AAD/keyring, DTO enmascarado y APIs tenant-scoped | Migrar/desplegar H3.1 en Preview, revisar legalmente el aviso y probar Meta E2E; completar Flow/UI de cobro, titularidad y comprobante. La purga no es DSAR y el teléfono raw en `Conversation.externalId` sigue como deuda |
...
| CI y E2E | En implementación | 1.369 tests locales, lint y build verdes; E2E público separado y workflow CI agregados | Requiere ejecución remota verde y expansión a journeys autenticados/Meta/storage/visión |
...
| Calidad automatizada | CI remota verde con instalación exacta, lint, suite completa, auditoría, build y smoke público | 1.369/1.369, Prisma, lint, build y auditoría de dependencias de producción verdes localmente; ejecución GitHub y E2E remoto pendientes |
...
| WhatsApp real | Embedded Signup, inbound/outbound, estados, Flows operativos y pre-operario, reintento, expiración y fallback en teléfono real | Test number asignado, celular propio verificado y outbound de plantilla aceptado por Meta con token temporal. No es bidireccional/E2E: faltan credenciales permanentes en Vercel, webhook firmado, inbound, estados, Flows, App Review y tenant real |
...
- verificados en este WIP local: 1.369/1.369 tests, Prisma válido/generado, lint, build y auditoría de dependencias de producción sin vulnerabilidades; CI remota, migración desde cero en Postgres y E2E deben volver a ejecutarse después de integrar el sprint;
...
Estado local de identidad laboral/cobro al corte: H3.1 ya incluye invitación, Flow/sesión pre-operario, aviso fijado, acuse terminal, CRM/readiness, decisión y purga transitoria, además del dominio criptográfico y APIs tenant-scoped. Su migración todavía no fue desplegada ni verificada en Neon Preview y no existe E2E Meta. `privacyPresentedAt` prueba que `INIT` sirvió el aviso, no lectura humana; el copy requiere revisión legal antes de trabajadores reales. La purga H3.1 no es DSAR integral y quedan fuera `WorkerPerson`, `WorkerChannelIdentity`, `Worker`, mensajes y backups; también sigue pendiente retirar el teléfono raw interno de `Conversation.externalId`.
