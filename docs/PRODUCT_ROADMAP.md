...
| --- | --- |
| Commit/CI reproducible | El commit `871cf2f` contiene 121 migraciones. CI `31511714114` cerró 3/3: PostgreSQL 17 aprobó PRO-05A y PRO-05B.1 con carreras disposable, status limpio y drift cero; el job general aprobó `2230/2230` pruebas, lint, auditoría de producción y build Next.js 16.2.11 de 88 páginas. Los jobs y límites están fijados en la [evidencia PRO-05B.1](./evidence/2026-08-11-preview-871cf2f.md). El [corte S12.2C `fc71fbe`](./evidence/2026-08-11-preview-fc71fbe.md) permanece como evidencia histórica independiente |
| Último Preview reproducible | El deployment inmutable `dpl_Fj7zRe66SrNtaeLXQSXhESjygPSs` del commit `871cf2f` quedó `Ready`: Neon detectó 121 migraciones sin pendientes, S12.2C pasó rollback-only sin carreras disposable, PRO-05A y PRO-05B.1 quedaron verdes y el build cerró 88/88. Los smokes read-only dieron `/`, `/privacy` y `/sign-in` `200`; sin sesión, `/dashboard/privacy` `404`. Un `ADMIN` sintético vio la consola no ejecutable y cola vacía; un `AUDITOR` recibió el boundary restringido y no vio el enlace. No hubo POST ni mutación, y en la ventana de 30 minutos hubo cero `error`, `fatal` o `5xx`. No se movió ni se certifica un alias. No acredita DSAR ejecutable, cross-tenant, Meta E2E, trabajadores reales ni Production ([evidencia](./evidence/2026-08-11-preview-871cf2f.md)) |
