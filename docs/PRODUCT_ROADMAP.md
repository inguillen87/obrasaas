...
| --- | --- |
| Commit/CI reproducible | El commit `46c744c` contiene 122 migraciones. CI `31528997881` cerró 3/3: PostgreSQL 17 aprobó S9.1 con journey gobernado, carreras disposable y cleanup exacto, además de PRO-05A/PRO-05B.1, status limpio y drift cero; quality aprobó `2294/2294` pruebas, lint, auditoría de producción sin vulnerabilidades y build Next.js 16.2.11 de 89 páginas. Los jobs y límites están fijados en la [evidencia S9.1](./evidence/2026-08-11-preview-46c744c.md). Los cortes [PRO-05B.1 `871cf2f`](./evidence/2026-08-11-preview-871cf2f.md) y [S12.2C `fc71fbe`](./evidence/2026-08-11-preview-fc71fbe.md) permanecen como evidencia histórica independiente |
| Último Preview reproducible | El deployment inmutable `dpl_7GtJuVDAZiYd1R95eybxnGSfpFie` del commit `46c744c` quedó `READY`: Neon detectó 122 migraciones sin pendientes, S9.1 pasó rollback-only sin carreras disposable, S12.2C, PRO-05A y PRO-05B.1 quedaron verdes y el build cerró 89/89. Los smokes públicos read-only dieron `/`, `/privacy`, `/terms` y `/data-deletion` `200`; en los logs exactos observados hubo cero `error`, `fatal` o `5xx`. No hubo journey autenticado S9.1, POST ni mutación de datos, no se movió un alias y no acredita Production ([evidencia](./evidence/2026-08-11-preview-46c744c.md)) |
...
| Medición técnica de avance | S9.1 verificado en CI PostgreSQL 17 y Preview; journey autenticado pendiente | [Ledger cuantitativo](./PROGRESS_MEASUREMENTS_S9_1.md) por tarea y quincena civil con unidad, base, cantidad ejecutada, evidencia aprobada, Decimal exacto, CAS, replay inmutable y maker-checker. No modifica `Task.progress`, baseline, forecast, presupuesto, certificado ni pago | Recorrer el journey autenticado por rol y construir S9.2-MED: corte técnico quincenal reproducible a nivel obra. No abrir S10 sobre proyecciones vivas |
...
| CI y E2E | Parcial con gate remoto reproducible | El corte `871cf2f` aprobó `2230/2230` tests, Prisma, lint, build 88/88, auditoría sin vulnerabilidades, PostgreSQL 17 con 121 migraciones/drift cero y Playwright público. El smoke PRO-05B.1 comprobó acceso `ADMIN`, rechazo `AUDITOR` y signed-out fail-closed, todo read-only; la evidencia S12.2C multirrol anterior se conserva separada | Recorrer POST sintético maker-checker, cross-tenant y journeys core/Meta/media/storage/visión; no confundir CI verde ni una consola accesible con DSAR ejecutable o Production |
...
| Calidad automatizada | CI remota verde con instalación exacta, lint, suite completa, auditoría, build y smoke público | `871cf2f`: `2230/2230`, Prisma, lint, auditoría con cero vulnerabilidades, build 88/88, PostgreSQL 17 con 121 migraciones/drift cero, PRO-05A/B.1 y Playwright público; CI GitHub `31511714114` terminó 3/3 verde |
...
| S9.2-MED | Corte técnico quincenal | snapshot inmutable de todas las mediciones aprobadas de la obra y período, con hash, versión y replay | una corrección posterior no reescribe el corte; el siguiente corte explicita la nueva composición |
