...
**Corte de evidencia:** 10 de agosto de 2026
...
| --- | --- |
| WIP local | El commit `edeea82` contiene 119 migraciones y pasa `2127/2127` pruebas, lint, Prisma, auditoría de dependencias de producción y build Next.js 16.2.11 de 88 páginas. CI exige ahora el verificador PRO-05A en PostgreSQL 17 y Vercel lo ejecuta con la conexión Preview aprobada antes del `prisma generate` gobernado y del build Next. S12.2B todavía no tiene journey autenticado y S12.2C continúa sin implementar |
| Último Preview reproducible | El commit `edeea82` quedó `Ready`: Neon detectó 119 migraciones sin pendientes, repitió la reconciliación estructural y aprobó PRO-05A antes del generate gobernado y del build 88/88. CI cerró sus tres jobs. El smoke autenticado del alias con un AUDITOR sintético confirmó `/dashboard` permitido; Inbox, Integraciones y Equipo mostraron el límite sanitizado sin datos privados, aunque conservan HTTP `200` por streaming; Presupuesto y Superadmin respondieron `403`. No acredita matriz completa de roles, reservas, Meta E2E ni Production ([evidencia](./evidence/2026-08-10-preview-edeea82.md)) |
...
| Materiales y stock | S12.2A y S12.2B DB/build en Preview; UI autenticada pendiente | [Ledger on-hand](./INVENTORY_STOCK_LEDGER.md) con putaway/reversión y balance DB-owned; [BOM por tarea](./TASK_MATERIAL_REQUIREMENTS.md) versionada, append-only, con cantidades exactas y declaración explícita sin materiales. Las migraciones 116+117 y el build pasaron en Preview | Recorrer S12.2B por roles y construir S12.2C. `SupplierCommitment` es **PROMESA, NO RESERVA**; `AVAILABLE` exige reserva exacta suficiente |
