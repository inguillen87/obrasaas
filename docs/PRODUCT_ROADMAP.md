...
| --- | --- |
| Commit/CI reproducible | El commit `fc71fbe` contiene 120 migraciones. CI PostgreSQL 17 aprobó S12.2C en modo disposable con cuatro carreras reales y cleanup exacto, PRO-05A, status limpio y drift cero; el job general aprobó `2172/2172` pruebas, lint, auditoría de producción y build Next.js 16.2.11 de 88 páginas. Los tres jobs y sus URLs están fijados en la [evidencia](./evidence/2026-08-11-preview-fc71fbe.md) |
| Último Preview reproducible | El deployment inmutable del commit `fc71fbe` quedó `Ready`: Neon detectó 120 migraciones sin pendientes, S12.2C pasó en modo rollback-only, PRO-05A volvió a quedar verde y el build cerró 88/88. `GET /` respondió `200`; durante el smoke autenticado y la ventana observada no hubo eventos `error`/`fatal` ni `5xx`. Un único usuario sintético recorrió `AUDITOR → DIRECTOR → SITE_MANAGER → AUDITOR`, validó boundaries de conversaciones, secciones restringidas y proyectos, y terminó restaurado y sin sesión. No se movió ni se certifica un alias estable. No acredita matriz completa de roles, reserva/liberación exitosa, cross-tenant, Meta E2E, trabajadores reales ni Production ([evidencia](./evidence/2026-08-11-preview-fc71fbe.md)) |
...
| Privacidad y derechos del titular | Gate técnico PRO-05A en Preview, discovery-only y `DISCOVERY_BLOCKED` | [Caso y manifiesto tenant-scoped](./DATA_SUBJECT_RIGHTS_FOUNDATION.md), atestación ADMIN que no suplanta identidad, catálogo v1 fijado, consultas read-only, HMAC sin identificadores fuente, replay exacto, rate limit durable y guardas DB append-only. El verificador conductual de `140000` pasa en PGlite, CI PostgreSQL 17 y Vercel/Neon Preview. No verifica al solicitante ni decide base legal; no exporta, corrige, restringe, ofrece portabilidad, anonimiza o elimina datos | Completar identidad/representación y decisiones PRO-05B, adapters PRO-05C, propagación y restore PRO-05D, matriz legal/entidad responsable y E2E sintético autorizado antes de trabajadores reales |
...
| CI y E2E | Parcial con gate remoto reproducible | El corte `fc71fbe` aprobó `2172/2172` tests, Prisma, lint, build 88/88, auditoría sin vulnerabilidades, PostgreSQL 17 con 120 migraciones/drift cero y Playwright público; el smoke sintético cubrió boundaries de tres roles | Expandir a journey funcional de materiales, rol restante, negativos cross-tenant, Meta/media/storage/visión y proveedores externos; no confundir CI verde con Production |
...
| Calidad automatizada | CI remota verde con instalación exacta, lint, suite completa, auditoría, build y smoke público | `fc71fbe`: `2172/2172`, Prisma, lint, auditoría con cero vulnerabilidades, build 88/88, PostgreSQL 17 con 120 migraciones/drift cero y Playwright público; CI GitHub `31452913235` terminó 3/3 verde |
| Journey operativo E2E | Admin crea obra/trabajador/tarea, aprueba propuesta y descarga reporte; roles restringidos fallan correctamente | Parcial: boundary sintético de `AUDITOR`, `DIRECTOR` y `SITE_MANAGER` aprobado; alta de trabajador, flujo core completo, rol restante y cross-tenant pendientes |
...
| Migraciones gobernadas | Postgres efímero, `migrate deploy`, estado, diff sin drift, smoke y rollback documentados | CI PostgreSQL 17 y Vercel/Neon Preview aprobaron 120 migraciones, verificadores y drift cero en `fc71fbe`; promoción atómica/drenaje y Production siguen pendientes; no admitir pods v1/v2 balanceados |
...
- Postgres 17 efímero y Neon Preview: `validate`, las 120 migraciones actuales, estado sin pendientes, verificadores semánticos y smoke rollback-only verdes en `fc71fbe`; el corte histórico `0a00f37` permanece como evidencia de sus migraciones 100 y gates de ese momento;
...
- verificados para `fc71fbe`: `2172/2172` tests, Prisma válido/generado, lint, build 88/88, auditoría de dependencias de producción sin vulnerabilidades, PostgreSQL 17 con 120 migraciones/drift cero, CI GitHub 3/3 y Vercel/Neon Preview `Ready`; el boundary autenticado de tres roles quedó verde, mientras el journey core, cross-tenant y E2E externo siguen abiertos;
...
[PRO-05A](./DATA_SUBJECT_RIGHTS_FOUNDATION.md) ya tiene una base no destructiva cuyo gate técnico fue verificado en Preview: registro tenant-scoped, atestación administrativa separada de la identidad del solicitante, discovery `READ ONLY`, catálogo y manifiesto inmutables, blockers obligatorios y rate limit durable. Su migración `140000` aplica desde cero con las 108 anteriores en PGlite; el mismo verificador conductual volvió a quedar verde en CI PostgreSQL 17 y Vercel/Neon Preview sobre el esquema completo de 120 migraciones del [corte `fc71fbe`](./evidence/2026-08-11-preview-fc71fbe.md). Esto no es un DSAR ejecutable: PRO-05 continúa **abierto** y v1 sólo puede terminar `DISCOVERY_BLOCKED`. PRO-05B debe incorporar identidad/representación, plazos, bases y holds; PRO-05C, acciones idempotentes por dominio; PRO-05D, terceros, backups y tombstone replay. Sin entidad legal/domicilio, matriz aprobada, E2E sintético del endpoint y esas fases no se habilitan datos de trabajadores reales.
