...
| --- | --- |
| Commit/CI reproducible | El commit `cc5aa21` contiene 123 migraciones. CI `31539591755` cerró 3/3: PostgreSQL 17 aprobó S9.2-MED rollback-only, replay exacto/mutado, carreras de dos selladores, corrección y archivado con cleanup exacto, además de S9.1, S12.2C y PRO-05A/B.1; `migrate status` quedó al día y el drift fue cero. Quality aprobó `2347/2347`, lint, auditoría de producción sin vulnerabilidades y build Next.js 16.2.11 de 90 páginas; Browser cerró 2/2. Los jobs y límites están fijados en la [evidencia S9.2-MED](./evidence/2026-08-11-preview-cc5aa21.md). Los cortes [S9.1 `46c744c`](./evidence/2026-08-11-preview-46c744c.md), [PRO-05B.1 `871cf2f`](./evidence/2026-08-11-preview-871cf2f.md) y [S12.2C `fc71fbe`](./evidence/2026-08-11-preview-fc71fbe.md) permanecen como evidencia histórica independiente |
| Último Preview reproducible | El deployment inmutable `dpl_4p2bcbwyznb4afZ8XetL2GrZWF51` del commit `cc5aa21` quedó `READY`: Neon detectó 123 migraciones sin pendientes, S9.2-MED pasó rollback-only con carreras disposable desactivadas, S9.1 y los gates PRO quedaron verdes y el build cerró 90/90. Los smokes públicos read-only dieron `/`, `/privacy`, `/terms` y `/data-deletion` `200`; en los ocho eventos runtime exactos observados hubo sólo nivel `info`, respuestas `200` y cero `error`, `fatal` o `5xx`. No hubo journey autenticado ni POST S9.2, no se movió un alias y no acredita Production ([evidencia](./evidence/2026-08-11-preview-cc5aa21.md)) |
...
| Medición técnica de avance | S9.1 y S9.2-MED verificados en CI PostgreSQL 17 y Preview; journey autenticado pendiente | El [ledger cuantitativo S9.1](./PROGRESS_MEASUREMENTS_S9_1.md) conserva unidad, base, cantidad ejecutada, evidencia aprobada, Decimal exacto y maker-checker. El [corte S9.2-MED](./PROGRESS_MEASUREMENT_CUTS_S9_2.md) sella todas las tareas canónicas de la obra/quincena como `MEASURED` o `MISSING`, con composición server-owned, versión, hashes, replay y doble CAS. Ninguno modifica `Task.progress`, baseline, forecast, presupuesto, certificado ni pago | Recorrer ambos journeys autenticados por rol. S10 puede consumir exclusivamente cortes técnicos sellados, pero debe crear una autoridad contractual/económica separada; S9.2 no es certificado ni instrucción de pago |
...
| CI y E2E | Parcial con gate remoto reproducible | El corte `cc5aa21` aprobó `2347/2347`, Prisma, lint, build 90/90, auditoría sin vulnerabilidades, PostgreSQL 17 con 123 migraciones/drift cero y Playwright público 2/2. S9.2-MED ejecutó carreras disposable y cleanup exacto en CI; Preview repitió el verificador rollback-only sin carreras. Los boundaries autenticados históricos se conservan separados | Recorrer S9.1/S9.2 autenticados, POST maker-checker de privacidad, matriz completa, cross-tenant y journeys core/Meta/media/storage/visión; no confundir CI verde ni un smoke público con operación real o Production |
...
| Calidad automatizada | CI remota verde con instalación exacta, lint, suite completa, auditoría, build y smoke público | `cc5aa21`: `2347/2347`, Prisma, lint, auditoría con cero vulnerabilidades, build 90/90, PostgreSQL 17 con 123 migraciones/drift cero, S9.1/S9.2-MED, S12.2C, PRO-05A/B.1 y Playwright público 2/2; CI GitHub `31539591755` terminó 3/3 verde |
| Journey operativo E2E | Admin crea obra/trabajador/tarea, aprueba propuesta y descarga reporte; roles restringidos fallan correctamente | Parcial: se conservan los boundaries históricos `AUDITOR`/`DIRECTOR`/`SITE_MANAGER`; PRO-05B.1 agregó acceso read-only `ADMIN`, rechazo `AUDITOR` y signed-out 404. No se ejecutó POST de privacidad, alta de trabajador, flujo core completo ni cross-tenant |
...
| Migraciones gobernadas | Postgres efímero, `migrate deploy`, estado, diff sin drift, smoke y rollback documentados | CI PostgreSQL 17 aprobó 123 migraciones y los gates S9.1/S9.2-MED, S12.2C y PRO-05A/B.1 en `cc5aa21`; cerró `migrate status` y drift cero. Vercel/Neon Preview detectó las mismas 123 sin pendientes y aprobó S9.1/S9.2-MED y los gates PRO aplicables; S9.2 usó `PROGRESS_MEASUREMENT_CUTS_DISPOSABLE_CONCURRENCY=0`, porque sus carreras y cleanup sólo corresponden al PostgreSQL descartable. Promoción atómica/drenaje y Production siguen pendientes; no admitir pods v1/v2 balanceados |
