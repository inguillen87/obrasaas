...
S9.1 agrega la [medición técnica cuantitativa](./PROGRESS_MEASUREMENTS_S9_1.md) por tarea y quincena civil. S9.2-MED agrega el [corte técnico reproducible](./PROGRESS_MEASUREMENT_CUTS_S9_2.md) de todas las tareas canónicas de la obra/período, con ausencias explícitas `MISSING`, versión, hashes, replay y doble CAS. El [corte técnico `cc5aa21`](./evidence/2026-08-11-preview-cc5aa21.md) verificó 123 migraciones, carreras PostgreSQL, cleanup, drift cero y Preview rollback-only. El [corte funcional `dcb44b9`](./evidence/2026-08-11-preview-dcb44b9.md) agregó CI 4/4 y el journey S9.2 API/UI con seis actores Clerk Development, maker-checker, roles negativos, cross-tenant, seal/replay/stale y PostgreSQL descartable; Preview permaneció read-only. S9.1/S9.2 no reescriben `Task.progress` y no crean certificado, importe, conformidad financiera, PDF contractual ni pago. S9.3-CONTRACT ya tiene [autoridad/SOV congelada como diseño](./PROJECT_CONTRACT_AUTHORITY_S9_3.md), separada de `BudgetVersion` y `SupplierInvoice`, pero no está implementada ni verificada. La UI dedicada completa de S9.1, S9.3 y S10 permanecen pendientes.
...
| SPEC-PLN-05 | Certificación de avance para quincena | Parcial; input técnico S9.1/S9.2 y E2E S9.2 verificados | Existe ledger cuantitativo por tarea/quincena y un corte de obra versionado que conserva cada tarea como `MEASURED` o `MISSING`; S9.2 recorrió maker-checker, seal/replay/stale, roles y cross-tenant en Development descartable. El diseño S9.3 fija una SOV `VALUED`/`NO_CLAIM`, moneda/minor units, retención y autoridades propias, pero no está implementado y no existe certificado contractual | Implementar/verificar S9.3 y luego S10-CERT, S10-FIN y S10-PAYREF como autoridades append-only distintas; completar aparte la UI dedicada S9.1 | S9.1-S10 |
...
| SPEC-AVA-04 | Certificado de avance | Ausente; inputs técnicos disponibles y S9.3 sólo documental | Existe medición aprobada versionada y corte técnico reproducible. El [freeze S9.3](./PROJECT_CONTRACT_AUTHORITY_S9_3.md) define SOV e importes contractuales propios, pero todavía no existen su persistencia ni un certificado | Implementar/verificar S9.3; después S10-CERT debe consumir cut + contrato exactos y crear un artefacto privado, versionado, inmutable y reproducible. Conformidad, referencia y PDF permanecen separados | S9.3-S10 |
...
| SPEC-REP-03 | Historial de certificaciones/quincenas | Ausente | S9.2 conserva historial de cortes técnicos por quincena y versión; S9.3 sólo congela el contrato de diseño y no existe certificación canónica ni PDF contractual | Historial de certificados por período, cut exacto, versión SOV, decisiones, importe y PDF, sin reutilizar el head técnico, `BudgetVersion` o `SupplierInvoice` | S9.3-S10 |
...
| SPEC-ALT-06 | Quincena por certificar → Director | Ausente | Hay período técnico S9.2 y autoridades S9.3 sólo diseñadas; no hay contrato implementado ni certificación canónica | Tras verificar S9.3/S10-CERT, crear borrador una vez, vencimiento, owner y escalamiento | S9.3-S10, S15 |
...
| SPEC-DEF-03 | Certificación: documento que valida avance para pago | Reformular | No existe certificado. S9.2 fija un corte técnico y S9.3 congela una base contractual aparte; la definición original mezcla certificado, conformidad y pago | Implementar S9.3 y dividir S10 en certificado contractual, conformidad financiera y referencia externa append-only. Ninguna fase crea `PAID` ni prueba pago | S9.3-S10 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial en Preview; S9.2 autenticado en CI Development | El [corte `dcb44b9`](./evidence/2026-08-11-preview-dcb44b9.md) terminó CI 4/4 con 123 migraciones, status/drift cero, `2388/2388`, build 90/90, Browser 2/2 y S9.2 autenticado 2/2. Seis actores en dos tenants recorrieron maker-checker, roles negativos, cross-tenant, seal/replay/stale y UI sobre PostgreSQL descartable. Vercel/Neon Preview quedó `READY` y read-only, sin sesión ni POST | Faltan S9.1 UI, POST maker-checker de privacidad, matriz global y E2E core/externos; el cross-tenant S9.2 está cubierto, pero ningún gate acredita Production | S0 |
...
| PRO-11 | Presupuesto, comprometido, real, forecast y cambios | Parcial, base local | Existen `BudgetVersion`, `BudgetLine`, `BudgetEntry` y clases `COMMITMENT/ACTUAL/FORECAST`; son presupuesto interno y no SOV contractual | Faltan cambios aprobados, reconciliación, cutover y Preview. S9.3 debe implementar su autoridad separada y S19 gobernar variaciones; nunca promover una BudgetVersion a contrato por equivalencia | S7, S9.3-S12, S19 |
