...
| SPEC-OBJ-02 | Geolocalización y tiempo real | Reformular | La app usa lecturas GPS puntuales y el contrato/copy probado declara que no hace seguimiento continuo | Hora servidor, accuracy, geocerca, consentimiento y excepción; nunca presentarlo como tracking o identidad infalible | S1-S2 |
...
| SPEC-AVA-02 | IA estima porcentaje por foto | Reformular | Adapter gobernado con rango o abstención, derivado sin metadatos y ejecución histórica controlada de `gpt-5.6-sol`; la clave dedicada ya está aislada y el nuevo dispatch conserva ruta/presupuesto/costo. El resultado se persiste primero como recibo inmutable y se reanuda sin redispatch; usage ausente conserva `costPending`. `store:false` no satisface ZDR por sí solo y el nuevo ledger/receipt aún debe desplegarse | Dataset consentido, ground truth, privacidad/DPA/retención, benchmark visual OpenAI/Qwen3-VL/GLM-5V por tipología y evals separadas de GLM-OCR/GLM-5.2 para OCR/texto; una foto aislada nunca certifica | S17 |
...
| SPEC-AVA-08 | Avance validado actualiza plan y habilita certificación | Reformular | Baseline/forecast existen, pero una foto o revisión visual no los actualiza automáticamente y el certificado sigue separado | Una decisión humana explícita puede solicitar un forecast; una medición posterior inicia certificación. Nunca habilita pago automáticamente | S6, S9-S10 |
...
| SPEC-TEC-03 | Nube para fotos/videos | Externo pendiente | Vercel Blob privado/Cloudinary autenticado y `ProtectedUpload` server-owned ya tienen evidencia base en Preview; validación binaria, intent durable y cron autenticado están implementados. El lifecycle durable de media WhatsApp `070000` sigue sólo local | Desplegar/verificar `070000`, observar ambiente/cron y media real, completar retención, restore, DR, carga directa para más de 4 MiB y degradación | S0, R0, S23 |
...
| PRO-12 | Identidad laboral y destino de cobro protegidos | Parcial | El módulo valida CUIL/CBU/CVU/alias, exige consentimiento versionado, cifra con AAD/keyring y serializa enmascarado. Ya existen API tenant-scoped, CAS, idempotencia, contrato/checks de titularidad y ledger append-only; sus migraciones se verificaron en Preview aislado. La verificación real no está operativa y la ruta falla cerrada con `503` sin un proveedor confiable | Completar Flow/UI, proveedor de titularidad, opt-in y comprobante privado; nunca ejecutar pagos desde IA/WhatsApp | Piloto H3-H4, S14, R0 |
