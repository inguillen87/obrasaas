...
Estado de los journeys pedidos: H2 tiene un contrato local de captura foto-primero con geolocalización opcional, pero su migración aún no fue aplicada en Neon Preview y no existe E2E real con Meta. H4 ya cierra localmente consentimiento, CRM, Data Endpoint terminal, caller inbound con emisión atómica de sesión base/companion y reconciliación DB-only de un resultado `UNCERTAIN` respaldado por procedencia exacta; sus cuatro migraciones `13000-13300` siguen sólo validadas en PGlite y faltan desplegar/verificar ese caller en Neon/Vercel Preview, Meta E2E y comprobante privado. H5 ya convierte localmente una revisión humana en una observación append-only y un forecast comparado contra baseline, sin mutarla; faltan migración y journey autenticado en Preview y E2E con foto Meta real.
...
| PRO-12 | Identidad laboral y destino de cobro protegidos | Parcial local | El módulo valida CUIL/CBU/CVU/alias, conserva aviso versionado, cifra con AAD/keyring y serializa enmascarado. H4 ya incorpora consentimiento específico append-only por destino/canal, re-atestación legacy, panel CRM enmascarado, companion Flow de un solo uso, Data Endpoint terminal y reconciliador DB-only `UNCERTAIN → SUCCEEDED` que exige procedencia exacta y no reejecuta efectos. El caller inbound local limita la emisión a identidad `CANONICAL` verificada, crea base/companion en la misma transacción del webhook y redacta datos financieros textuales antes de la cola. Sus cuatro migraciones `13000-13300` sólo pasaron PGlite local. Si la base no puede conciliar, el Data Endpoint devuelve `503` sin ciphertext y el journal queda inmediatamente reclamable, no bloqueado por una caché negativa de 24 horas. La verificación bancaria real falla cerrada con `503` sin proveedor | Desplegar y verificar el caller especializado en Neon y Vercel Preview, publicar/probar el Flow en Meta E2E, integrar proveedor de titularidad y comprobante privado; nunca verificar, activar, pagar ni reintentar resultados ambiguos desde IA/WhatsApp. No afirmar Production | Piloto H3-H4, S14, R0 |
