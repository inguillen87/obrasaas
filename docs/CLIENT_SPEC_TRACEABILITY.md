...
| SPEC-PLN-05 | Certificación de avance para quincena | Parcial; input técnico S9.1/S9.2 verificado | Existe ledger cuantitativo por tarea/quincena y un corte de obra versionado que conserva cada tarea como `MEASURED` o `MISSING`; no existe certificado contractual | Cerrar el journey autenticado y construir en S10 una autoridad separada para certificado, conformidad financiera y referencia de pago | S9-S10 |
...
| SPEC-DEF-02 | Avance: porcentaje de completitud | Parcial; medición/corte técnico S9.1/S9.2 verificados | `Task.progress` legacy permanece separado; S9.1 guarda método, cantidad base/ejecutada, período, evidencia, autor y maker-checker, y S9.2 sella la composición completa sin mutar la tarea | Recorrer UI autenticada; el porcentaje solo y el corte técnico no certifican | S5, S9 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial en Preview | El [corte `cc5aa21`](./evidence/2026-08-11-preview-cc5aa21.md) terminó CI 3/3 con 123 migraciones, S9.2-MED disposable, status y drift cero, `2347/2347`, build 90/90 y Browser 2/2; Vercel/Neon Preview repitió los gates rollback-only con carreras desactivadas y quedó `READY`. El smoke nuevo fue público/read-only, sin POST S9.2 ni sesión autenticada. Las evidencias históricas S9.1, PRO-05B.1 y S12.2C conservan sus alcances separados | Faltan journeys S9.1/S9.2 autenticados, POST maker-checker sintético, matriz completa, cross-tenant y E2E core; ningún gate acredita Production | S0 |
