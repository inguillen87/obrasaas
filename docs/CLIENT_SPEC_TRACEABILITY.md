...
| SPEC-MAT-05 | Stock actualizado | Parcial | Ya existe un catálogo tenant-scoped de ubicaciones activas, con tope y snapshot histórico en inspección; todavía no hay ledger de existencias | Saldo derivado de movimientos de recepción aceptada, consumo, transferencia y ajuste por ubicación | S12 |
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; aceptación en Preview, disponibilidad cerrada | Un compromiso vinculado a una tarea deriva readiness sin modificar `Task.status`. La recepción conciliada e inspeccionada ya distingue cantidad `ACCEPTED`, pero eso aún no prueba stock libre ni reserva. `FULFILL` administrativo sigue siendo `ADMIN_ATTESTED`, nunca `AVAILABLE` | Implementar ledger/reserva y BOM versionado por tarea. Sólo cantidad aceptada, existente y reservada suficiente puede derivar `AVAILABLE`; foto/OCR nunca habilita por sí solo | S12 |
