...
S12.2B agrega una [BOM inmutable por tarea](./TASK_MATERIAL_REQUIREMENTS.md). Sus migraciones 116+117, verificador PostgreSQL y build están [verificados en Preview](./evidence/2026-08-09-preview-054a82c.md). S12.2C agrega la [reserva/liberación exacta](./TASK_MATERIAL_RESERVATIONS.md) de toda la BOM vigente; la migración 120, cuatro carreras PostgreSQL y el gate rollback-only de Vercel/Neon quedaron [verificados para `fc71fbe`](./evidence/2026-08-11-preview-fc71fbe.md). El mismo artefacto aprobó boundaries sintéticos de `AUDITOR`, `DIRECTOR` y `SITE_MANAGER`; siguen pendientes una reserva/liberación exitosa, el rol restante y negativos cross-tenant. `AVAILABLE` sólo afirma materiales de la BOM actual completamente reservados, no ejecutabilidad, certificación ni pago.

S9.1 agrega la [medición técnica cuantitativa](./PROGRESS_MEASUREMENTS_S9_1.md)
por tarea y quincena civil. S9.2-MED agrega el [corte técnico
reproducible](./PROGRESS_MEASUREMENT_CUTS_S9_2.md) de todas las tareas canónicas
de la obra/período, con ausencias explícitas `MISSING`, versión, hashes, replay
y doble CAS. S9.3-CONTRACT agrega una [autoridad/SOV
versionada](./PROJECT_CONTRACT_AUTHORITY_S9_3.md), separada de `BudgetVersion` y
`SupplierInvoice`, con todas las tareas `VALUED`/`NO_CLAIM`, importes exactos,
retención, maker-checker y tres autoridades distintas. El [corte
`21ce752`](./evidence/2026-08-12-preview-21ce752.md) cerró CI 4/4, 124
migraciones sin drift, Quality 2446/2446, Browser público, E2E autenticado S9.2
y S9.3 2/2 cada uno, y Preview exact-SHA `READY`. Las mutaciones autenticadas
ocurrieron sólo sobre PostgreSQL descartable; Preview permaneció read-only.
S9.1/S9.2 no reescriben `Task.progress`; S9.3 no crea certificado, conformidad
financiera, PDF contractual ni pago. El [backend/API privada S10-CERT Fase
1](./PROJECT_CERTIFICATES_S10_CERT.md) quedó [cerrado técnicamente en
`8a3f446`](./evidence/2026-08-12-preview-8a3f446.md), con CI 4/4, 126
migraciones sin drift, E2E autenticado S10-CERT 2/2 y Preview rollback-only
`READY`. La UI S10, la UI dedicada completa de S9.1, S10-FIN, S10-PAYREF, PDF
y Production permanecen pendientes.
...
| S9.1-S9.2 | Medición de avance y corte técnico reproducible |
| S9.3-CONTRACT | Autoridad contractual/SOV versionada, separada de presupuesto interno y AP |
| S10 | Certificado, conformidad financiera y referencia externa de pago como tres fases distintas |
...
| SPEC-PLN-05 | Certificación de avance para quincena | Parcial; backend S10-CERT Fase 1 verificado, UI y fases posteriores pendientes | Existe ledger cuantitativo por tarea/quincena, corte versionado `MEASURED`/`MISSING`, SOV contractual `VALUED`/`NO_CLAIM` y certificado monetario versionado con autoridades propias. S9.2, S9.3 y S10-CERT recorrieron maker-checker, replay, roles y aislamiento tenant/obra sobre Clerk Development y PostgreSQL descartable; el [corte `8a3f446`](./evidence/2026-08-12-preview-8a3f446.md) cerró CI/Preview exactos | Diseñar y verificar la UI S10; mantener S10-FIN, S10-PAYREF y PDF como autoridades/artefactos separados; completar aparte la UI dedicada S9.1 | S9.1-S10 |
...
| SPEC-AVA-04 | Certificado de avance | Parcial; backend canónico verificado, UI ausente | S10-CERT consume cut S9.2 + contrato/autoridad S9.3 exactos y crea un snapshot monetario privado, versionado, inmutable y reproducible con preparación/decisión separadas. CI Development y Preview exact-SHA están verdes; no existe UI S10 ni se atribuye validez legal, contable o fiscal | Construir journey UI por rol y mantener conformidad S10-FIN, referencia S10-PAYREF y PDF como fases separadas | S10 |
...
| ID | Requisito original | Estado | Evidencia y decisión | Gap / criterio de aceptación | Secuencia |
| --- | --- | --- | --- | --- | --- |
| SPEC-MAT-01 | Materiales requeridos según tarea | Parcial en Preview; gate técnico y boundary multirrol, journey exitoso pendiente | `TaskMaterialRequirementRevision` publica una BOM versionada y append-only con snapshots y cantidades `Decimal(14,3)`; `NO_MATERIALS_REQUIRED` sigue explícito. S12.2C vincula la revisión vigente a un ledger append-only `RESERVE → RELEASE`, con CAS, idempotencia y cobertura exacta del bundle completo. Los gates remotos están en [S12.2B](./evidence/2026-08-09-preview-054a82c.md) y [S12.2C](./evidence/2026-08-11-preview-fc71fbe.md); el segundo también acredita límites de tres roles y rechazo seguro | Recorrer una reserva/liberación válida con roles de gestión, el rol restante y negativos cross-tenant. No existen sustitución, reserva parcial ni FIFO; consumo/devolución/transferencia/ajuste pertenecen al corte siguiente | S11-S12 |
| SPEC-MAT-02 | Pedido u orden a proveedor | Parcial en Preview | Proveedores, OC y líneas canónicas existen. OC y compromisos usan cantidades `Decimal(14,3)` exactas como texto, los importes de OC usan centavos exactos y la UI permite comprometer una partida y cantidad sin pasar por `Number`. `SupplierCommitment` agrega fecha/ventana y vínculo opcional con OC/tarea, pero es **PROMESA, NO RESERVA** | Recorrer UI por rol; completar requisición → comparación/selección → OC, edición multi-partida y segregación. Nunca compra automática ni cobertura inferida de la BOM | S11 |
...
| SPEC-MAT-05 | Stock actualizado | Parcial; S12.2A/B/C con gate técnico y boundary multirrol en Preview, subjourneys pendientes | S12.2A agrega catálogo/ubicación tenant-scoped, putaway de disposiciones `ACCEPTED`, ledger append-only, reversión espejo y on-hand DB-owned. S12.2B define la BOM. S12.2C agrega `InventoryAvailability` y saldos/reservas DB-owned sin sustituir `InventoryBalance`; [migración 120, concurrencia y límites de tres roles](./evidence/2026-08-11-preview-fc71fbe.md) quedaron verdes | Recorrer putaway, BOM y reserva/liberación válida con sesiones sintéticas; agregar consumo, devolución, transferencia, ajuste y cutover legacy con rollback. No existen sustitución, reserva parcial ni FIFO. No afirmar Production | S12 |
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; `AVAILABLE` implementado con alcance estricto, journey exitoso pendiente | `ACCEPTED` sólo pasa a on-hand tras putaway. S12.2C deriva `AVAILABLE` únicamente cuando todas las líneas de la BOM vigente están exactamente reservadas en materiales/ubicaciones activos y las proyecciones son coherentes. El boundary multirrol rechazó requests inválidos, pero no materializó `AVAILABLE`. Un `SupplierCommitment` es **PROMESA, NO RESERVA**; `FULFILL`, foto, OCR o email no habilitan el estado | Probar reserva/liberación válida por rol y degradación a `REVIEW_REQUIRED`. `AVAILABLE` no modifica `Task.status` ni prueba dependencias, cuadrilla, equipos, permisos, seguridad, avance, certificación o pago | S12 |
...
| --- | --- | --- | --- | --- | --- |
| SOC-OPS-01 | Organizar tareas por quincenas y asociarlas a un calendario | Parcial en Preview | La pantalla de Compras agrupa las tareas canónicas de los próximos 90 días y las fechas de material/servicio en quincenas civiles reales 1–15/16–fin de mes; el mismo read model entrega JSON y `.ics` autenticado. La migración y el build pasaron en Preview; declara truncamiento y el export falla cerrado si perdería datos | Recorrer UI autenticada e importar/reimportar en clientes soportados. No llamarlo sincronización: el `.ics` es snapshot, no feed revocable ni OAuth | S3, S15, R0 |
| SOC-PROC-01 | Registrar fecha prometida de material/aberturas o servicio y hacerla visible al equipo | Parcial en Preview | `SupplierCommitment` registra proveedor, tipo material/servicio, ventana civil, OC/tarea opcionales, estados, CAS, idempotencia y eventos. La UI captura cantidad exacta, concilia recepción, inspecciona por ubicación y cierra/revierte faltantes con historia; es contexto **PROMESA, NO RESERVA**. La reserva S12.2C se calcula sólo desde BOM y stock, no desde la promesa | Completar smoke por rol y UI multi-partida; no inferir cobertura ni mutar la tarea | S11-S12, R0 |
| SOC-PROC-02 | Enviar al proveedor un email una semana antes de ejecutar el servicio | Código en Preview, externo pendiente | Outbox dedicado, cron cada 15 minutos, envío Resend con idempotencia persistida, estados inciertos fail-closed y webhook firmado. La UI configura por defecto siete días. Rutas y build están en Preview, pero Resend/dominio no están configurados ni verificados; la marca de email es atestación de Administración, no doble opt-in del proveedor | Verificar dominio/remitente, variables, webhook, cron y ciclo accepted/delivered en staging; probar timeout, 429, bounce, complaint, supresión, reprogramación y owner de incidentes antes de afirmar envío real o Production | S15, R0 |
...
| SPEC-REP-03 | Historial de certificaciones/quincenas | Parcial; historial backend canónico, UI ausente | S9.2 conserva cortes técnicos por quincena/versión, S9.3 conserva versiones/decisiones de la SOV y S10-CERT agrega book/head/version/líneas/deducciones/decisiones/recibos append-only con lectura privada acotada. No existe UI de historial ni PDF contractual | Implementar historial UI por período, cut exacto, versión SOV, decisiones e importes; PDF queda en una fase posterior separada, sin reutilizar el head técnico, `BudgetVersion` o `SupplierInvoice` | S10 |
...
| SPEC-ALT-06 | Quincena por certificar → Director | Ausente como alerta | S10-CERT ya aporta certificación canónica backend y autoridad `DIRECTOR`, pero no existe evento durable, vencimiento, owner, escalamiento ni UI S10 | Crear la alerta desde readiness server-owned, una vez por período, con vencimiento, owner, resolución y escalamiento | S10, S15 |
...
| SPEC-DEF-02 | Avance: porcentaje de completitud | Parcial; medición/corte técnico S9.1/S9.2 y E2E S9.2 verificados | `Task.progress` legacy permanece separado; S9.1 guarda método, cantidad base/ejecutada, período, evidencia, autor y maker-checker, y S9.2 sella la composición completa sin mutar la tarea. `AUDITOR` verificó en UI la versión 2 sin permiso de sellado | Completar la UI dedicada S9.1; el porcentaje solo y el corte técnico no certifican | S5, S9 |
| SPEC-DEF-03 | Certificación: documento que valida avance para pago | Reformular; backend S10-CERT Fase 1 verificado | S9.2 fija un corte técnico, S9.3 una base contractual aprobada y S10-CERT materializa un snapshot monetario gobernado. El journey sintético preparó, rechazó, volvió a preparar y aprobó versiones; eso no otorga validez legal/contable/fiscal ni prueba conformidad o pago | Agregar UI S10 y conservar S10-FIN/conformidad, S10-PAYREF/referencia externa y PDF como fases separadas. Ninguna crea `PAID` ni prueba pago | S10 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial en Preview; S9.2/S9.3 autenticados en CI Development | El [corte `21ce752`](./evidence/2026-08-12-preview-21ce752.md) terminó CI 4/4 con 124 migraciones, status/drift cero, `2446/2446`, build 93/93, Browser público y S9.2/S9.3 autenticados 2/2 cada uno. Los journeys recorrieron maker-checker, roles negativos, tenant/obra, replay y UI/API sobre PostgreSQL descartable. Vercel/Neon Preview quedó `READY` y read-only, sin sesión ni POST | Faltan S9.1 UI, POST maker-checker de privacidad, matriz global y E2E core/externos; los límites S9.2/S9.3 están cubiertos, pero ningún gate acredita Production | S0 |
| PRO-04 | Observabilidad, SLO, alertas y runbooks | Parcial en Preview | El cron de recordatorios expone métricas sanitizadas y estado `healthy/degraded`; existe [runbook específico](./SUPPLIER_COMMITMENTS_AND_FORTNIGHT_CALENDAR.md). En la ventana de 30 minutos posterior al smoke PRO-05B.1, el deployment exacto registró cero `error`, `fatal` y HTTP `5xx`; esa muestra acotada no es un SLO. Todavía no hay ejecución real del cron, alertas conectadas ni evidencia Resend | S0, S15, S23 |
| PRO-05 | Retención, DSAR, backup/restore, RPO/RTO y borrado verificable | Parcial en Preview; revisión no ejecutable | [PRO-05A](./DATA_SUBJECT_RIGHTS_FOUNDATION.md) conserva caso/discovery tenant-scoped, read-only y bloqueado. El [plano PRO-05B.1](./DATA_SUBJECT_DECISION_CONTROL_PLANE.md) agrega identidad/representación versionada, revisión legal, holds, decisión por ítem, CAS, replay y maker-checker; CI/Preview y boundaries read-only quedaron verdes en `871cf2f`. `executionAllowed` permanece `false`: no exporta, corrige, restringe, porta, anonimiza ni elimina. Faltan POST sintético poblado, PRO-05C/D, entidad/matriz legal, proveedores, backups y restore | Gate previo a datos laborales reales; PRO-05B.1 no es DSAR ejecutable. S0, S23 |
...
| PRO-11 | Presupuesto, comprometido, real, forecast y cambios | Parcial, base local | Existen `BudgetVersion`, `BudgetLine`, `BudgetEntry` y clases `COMMITMENT/ACTUAL/FORECAST`; son presupuesto interno y no la SOV contractual S9.3 ya verificada | Faltan cambios aprobados, reconciliación, cutover y Preview del presupuesto. S19 debe gobernar variaciones; nunca promover una BudgetVersion a contrato por equivalencia | S7, S10-S12, S19 |
...
2. ¿La socia/cliente ratifica la política S9.3 congelada — SOV 1:1 por tarea, `VALUED`/`NO_CLAIM`, moneda/minor units, retención bps, redondeo y tres autoridades distintas — y qué revisión legal requiere antes de activarla?
