...
| SPEC-MAT-03 | Recepción con foto y remito firmado | Parcial en Preview | `GoodsReceipt` admite líneas parciales exactas, impide sobre-recepción, actualiza la OC y vincula remito privado. El ledger append-only asigna explícitamente cantidades de cada línea recibida a una línea comprometida, con saldos server-owned, aislamiento compuesto y sin FIFO | Completar inspección aceptado/rechazado/dañado/cuarentena, firma jurídica, retención, receptor/lugar visibles, evidencia tipada y E2E autenticado | S12 |
| SPEC-MAT-04 | Registrar faltantes de entrega | Parcial en Preview; inspección y Resend pendientes | El sistema compara exactamente cantidad comprometida y recepción explícitamente asignada, con estados `NOT_RECEIVED/PARTIALLY_RECEIVED/FULLY_RECEIVED`; fecha prometida, riesgo y recordatorio externo permanecen versionados | El faltante sólo puede cerrarse al finalizar la entrega; completar daño/sobrante/rechazo, owner, SLA, alerta interna a Compras/Director y evidencia Resend real | S11-S12, S15 |
...
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; conciliación en Preview, disponibilidad cerrada | Un compromiso vinculado a una tarea deriva readiness sin modificar `Task.status`; la recepción ya puede conciliarse cuantitativamente. Eso prueba asignación documental, no aceptación ni stock. `FULFILL` administrativo sigue siendo `ADMIN_ATTESTED`, nunca `AVAILABLE` | Implementar inspección, ledger/reserva y BOM versionado por tarea. Sólo cantidad aceptada y reservada suficiente puede derivar `AVAILABLE`; foto/OCR nunca habilita por sí solo | S12 |
...
| SOC-PROC-01 | Registrar fecha prometida de material/aberturas o servicio y hacerla visible al equipo | Parcial en Preview | `SupplierCommitment` registra proveedor, tipo, ventana, OC/tarea opcionales, estados, CAS, idempotencia y eventos. La UI captura cantidad exacta y ahora concilia explícitamente cada recepción con el compromiso; la disponibilidad continúa fail-closed y no muta la tarea | Completar smoke por rol, UI multi-partida, paginación, inspección/aceptación y cierre explícito de faltantes | S11-S12, R0 |
