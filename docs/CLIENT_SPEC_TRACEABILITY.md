...
| SPEC-MAT-02 | Pedido u orden a proveedor | Parcial en Preview | Proveedores, OC y líneas canónicas existen. `SupplierCommitment` agrega una fecha/ventana prometida para entrega o servicio, con vínculo opcional a OC aprobada/parcial y tarea canónica; migración y verificador ya pasaron en Neon/Vercel Preview. No hay requisición/BOM, comparación de cotizaciones ni segregación completa | Recorrer UI por rol y completar requisición → comparación/selección → OC aprobada y distribución cuantitativa usable por partida. Nunca compra automática | S11 |
...
| SPEC-MAT-04 | Registrar faltantes de entrega | Parcial en Preview; Resend pendiente | El sistema calcula `PARTIALLY_RECEIVED/RECEIVED`, evita sobre-recepción por línea de OC y modela en Preview la fecha prometida, estados `TENTATIVE/CONFIRMED/AT_RISK/FULFILLED/CANCELLED` y recordatorio externo versionado | Falta detección automática de incumplimiento contra recepción, daño/exceso/rechazo, pendiente, owner, SLA, alerta interna a Compras/Director y evidencia Resend real | S11-S12, S15 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial | El gate aplicó y verificó remotamente la migración de proveedores en Neon/Vercel Preview tras corregir el probe PostgreSQL `42P18`; smoke autenticado portable y E2E core siguen abiertos | S0 |
