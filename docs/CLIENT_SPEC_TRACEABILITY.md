...
S12.2B agrega una [BOM inmutable por tarea](./TASK_MATERIAL_REQUIREMENTS.md). Sus migraciones 116+117, verificador PostgreSQL y build están [verificados en Preview](./evidence/2026-08-09-preview-054a82c.md), pero el journey UI/API autenticado por rol sigue pendiente; no reserva ni declara materiales disponibles.
...
| ID | Requisito original | Estado | Evidencia y decisión | Gap / criterio de aceptación | Secuencia |
| --- | --- | --- | --- | --- | --- |
| SPEC-MAT-01 | Materiales requeridos según tarea | Parcial en Preview; DB/build verificado, UI autenticada pendiente | `TaskMaterialRequirementRevision` y sus líneas publican una BOM explícita, versionada y append-only por tarea canónica, con snapshots de tarea/material/unidad, cantidad `Decimal(14,3)`, motivo, actor, CAS, idempotencia e historial cursorado. También existe `NO_MATERIALS_REQUIRED`; `20260802180000_task_material_requirements` y `20260809090000_task_material_requirement_eligibility_not_null` pasaron el [verificador PostgreSQL y build de Preview](./evidence/2026-08-09-preview-054a82c.md) | Recorrer UI/API autenticada por roles de lectura y gestión. Implementar S12.2C para reserva/liberación; S12.2B conserva siempre `available: false` | S11-S12 |
...
| SPEC-MAT-05 | Stock actualizado | Parcial; S12.2A y S12.2B DB/build en Preview, journeys autenticados pendientes | S12.2A agrega catálogo/ubicación tenant-scoped, vínculo inmutable de línea, putaway atómico desde todas y sólo las disposiciones `ACCEPTED`, ledger append-only, reversión espejo y balance on-hand DB-owned. La [migración 115 y sus gates](./evidence/2026-08-02-preview-4760a50.md) pasaron en Preview. S12.2B define requerimientos por tarea y sus [migraciones 116+117 y build](./evidence/2026-08-09-preview-054a82c.md) también pasaron en Preview, pero no cambia el saldo | Recorrer putaway/reversión y S12.2B en UI/API autenticada; agregar reserva, consumo, devolución, transferencia, ajuste y cutover legacy con rollback. No afirmar Production | S12 |
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; aceptación/on-hand y BOM DB/build en Preview, `AVAILABLE` cerrado | La recepción `ACCEPTED` sólo pasa a existencia física tras putaway explícito. La revisión S12.2B puede quedar `DEFINED_UNRESERVED`, pero todos sus estados devuelven `available: false`. Un `SupplierCommitment` es **PROMESA, NO RESERVA** y no modifica `Task.status` | Recorrer el journey autenticado e implementar S12.2C con reserva/liberación exacta. Sólo cantidad aceptada, on-hand y reservada suficiente puede derivar `AVAILABLE`; `FULFILL`, foto, OCR o email nunca habilitan por sí solos | S12 |
...
| SOC-PROC-01 | Registrar fecha prometida de material/aberturas o servicio y hacerla visible al equipo | Parcial en Preview | `SupplierCommitment` registra proveedor, tipo material/servicio, ventana civil, OC/tarea opcionales, estados, CAS, idempotencia y eventos. La UI captura cantidad exacta, concilia recepción, inspecciona por ubicación y cierra/revierte faltantes con historia; es contexto **PROMESA, NO RESERVA** | Completar smoke por rol y UI multi-partida. La reserva pertenece a S12.2C; no inferir cobertura ni mutar la tarea | S11-S12, R0 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial en Preview | El corte `edeea82` exige PRO-05A en CI PostgreSQL 17 y Vercel/Neon Preview; detectó 119 migraciones sin pendientes, repitió reconciliación estructural y terminó `Ready`. El smoke autenticado con AUDITOR confirmó Dashboard permitido y límites sanitizados: Inbox/Integraciones/Equipo no filtraron datos pero mantienen HTTP `200` por streaming; Presupuesto/Superadmin respondieron `403`. Falta matriz completa, cross-tenant y E2E core | S0 |
