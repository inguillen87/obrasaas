...
financiera, PDF contractual ni pago. El [contrato S10-CERT Fase
1](./PROJECT_CERTIFICATES_S10_CERT.md) está congelado, pero su backend, la UI
dedicada completa de S9.1 y las fases posteriores de S10 permanecen pendientes.
...
| SPEC-PLN-05 | Certificación de avance para quincena | Parcial; inputs S9.1/S9.2/S9.3 verificados y contrato S10-CERT Fase 1 congelado | Existe ledger cuantitativo por tarea/quincena, corte versionado `MEASURED`/`MISSING` y SOV contractual `VALUED`/`NO_CLAIM` con moneda/minor units, retención y autoridades propias. S9.2 y S9.3 recorrieron maker-checker, replay, roles y aislamiento tenant/obra en Development descartable. [S10-CERT Fase 1](./PROJECT_CERTIFICATES_S10_CERT.md) define exactitud, actores, locks, FKs y gates; todavía no existe su backend ni certificado emitido | Implementar y verificar S10-CERT; mantener S10-FIN y S10-PAYREF como autoridades append-only distintas; completar aparte la UI dedicada S9.1 | S9.1-S10 |
...
| SPEC-AVA-04 | Certificado de avance | Ausente; inputs S9.1/S9.2/S9.3 disponibles | Existe medición aprobada versionada, corte técnico reproducible y [SOV contractual S9.3](./PROJECT_CONTRACT_AUTHORITY_S9_3.md) persistida, aprobable y versionada. No existe todavía un certificado | S10-CERT debe consumir cut + contrato exactos y crear un artefacto privado, versionado, inmutable y reproducible. Conformidad, referencia y PDF permanecen separados | S10 |
...
| SPEC-REP-03 | Historial de certificaciones/quincenas | Ausente; contrato S10-CERT Fase 1 congelado | S9.2 conserva historial de cortes técnicos por quincena/versión y S9.3 conserva versiones/decisiones de la SOV contractual. El contrato S10-CERT define book/head/version/decisión append-only, pero todavía no existe certificación canónica ni PDF contractual | Implementar historial de certificados por período, cut exacto, versión SOV, decisiones e importes; PDF queda en una fase posterior separada, sin reutilizar el head técnico, `BudgetVersion` o `SupplierInvoice` | S10 |
...
| SPEC-ALT-06 | Quincena por certificar → Director | Ausente | Hay período técnico S9.2 y contrato/autoridades S9.3 implementados; no hay certificación canónica ni evento durable | Tras implementar S10-CERT, crear borrador una vez, vencimiento, owner y escalamiento | S10, S15 |
...
| SPEC-DEF-03 | Certificación: documento que valida avance para pago | Reformular; contrato S10-CERT Fase 1 congelado | No existe certificado emitido. S9.2 fija un corte técnico, S9.3 una base contractual aprobada y S10-CERT ya congela el contrato backend del snapshot monetario; la definición original mezcla certificado, conformidad y pago | Implementar S10-CERT y conservar S10-FIN/conformidad y S10-PAYREF/referencia externa como fases append-only separadas. Ninguna crea `PAID` ni prueba pago | S10 |
