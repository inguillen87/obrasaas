...
S12.2B agrega localmente una [BOM inmutable por tarea](./TASK_MATERIAL_REQUIREMENTS.md), pero aún no fue aplicada en Neon/Vercel Preview y no reserva ni declara materiales disponibles.
...
| ID | Requisito original | Estado | Evidencia y decisión | Gap / criterio de aceptación | Secuencia |
| --- | --- | --- | --- | --- | --- |
| SPEC-MAT-01 | Materiales requeridos según tarea | Parcial local; S12.2B no Preview | `TaskMaterialRequirementRevision` y sus líneas publican una BOM explícita, versionada y append-only por tarea canónica, con snapshots de tarea/material/unidad, cantidad `Decimal(14,3)`, motivo, actor, CAS, idempotencia e historial cursorado. También existe `NO_MATERIALS_REQUIRED`; 27 pruebas focalizadas pasan localmente | Aplicar/verificar `20260802180000_task_material_requirements` en Neon/Vercel Preview y recorrer UI autenticada. Implementar S12.2C para reserva/liberación; S12.2B conserva siempre `available: false` | S11-S12 |
...
| SPEC-MAT-05 | Stock actualizado | Parcial; S12.2A Preview y S12.2B local | S12.2A agrega catálogo/ubicación tenant-scoped, vínculo inmutable de línea, putaway atómico desde todas y sólo las disposiciones `ACCEPTED`, ledger append-only, reversión espejo y balance on-hand DB-owned. La [migración 115 y sus gates](./evidence/2026-08-02-preview-4760a50.md) pasaron en Preview. S12.2B define requerimientos por tarea localmente, pero no cambia el saldo | Recorrer putaway/reversión en UI autenticada; aplicar S12.2B en Preview; agregar reserva, consumo, devolución, transferencia, ajuste y cutover legacy con rollback. No afirmar Production | S12 |
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; aceptación/on-hand en Preview, BOM local y `AVAILABLE` cerrado | La recepción `ACCEPTED` sólo pasa a existencia física tras putaway explícito. La revisión S12.2B puede quedar `DEFINED_UNRESERVED`, pero todos sus estados devuelven `available: false`. Un `SupplierCommitment` es **PROMESA, NO RESERVA** y no modifica `Task.status` | Implementar S12.2C con reserva/liberación exacta. Sólo cantidad aceptada, on-hand y reservada suficiente puede derivar `AVAILABLE`; `FULFILL`, foto, OCR o email nunca habilitan por sí solos | S12 |
