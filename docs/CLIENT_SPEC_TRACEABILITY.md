...
Estado de los journeys pedidos: H2 tiene un contrato local de captura foto-primero con geolocalización opcional, pero su migración aún no fue aplicada en Neon Preview y no existe E2E real con Meta. H4 sigue pendiente para datos de cobro/comprobante y H5 sigue pendiente para convertir una revisión humana en un escenario Gantt sin mutar la baseline.
...
| ID | Requisito original | Estado | Evidencia y decisión | Gap / criterio de aceptación | Secuencia |
| --- | --- | --- | --- | --- | --- |
| SPEC-PLN-01 | Gantt ideal con tareas, plazos y responsables | Parcial, motor entregado en Preview aislado | `Task`/`TaskDependency` son la autoridad canónica para el Gantt y usan revisión CAS; baseline inmutable/versionada y forecast determinista ya existen, y el snapshot queda como compatibilidad legacy | Falta cerrar smoke UI, responsable por ID y retirar el writer legacy sólo después de probar paridad/cutover | S3 |
...
| SPEC-PLN-03 | Plan real adaptativo por avances/retrasos | Parcial, motor entregado en Preview aislado | Forecast determinista con dependencias FS/SS/FF/SF, lag y baseline inmutable; la planificación relativa se rehidrata al fijar el calendario de obra | Cerrar smoke UI, escenarios aprobados/camino afectado y change control; baseline nunca se reescribe | S6 |
...
| SPEC-AVA-03 | Director aprueba/corrige estimación | Parcial | `VisualProgressAssessment` conserva resultado, revisión CAS y corrección/rechazo con motivo. Una recuperación ganadora antes del dispatch evita la llamada; después de esa frontera, un recibo durable puede aplicar una respuesta tardía sin redispatch. No muta el plan | Recorrer UI, replay y conciliación autenticados; unificar políticas de texto/audio/foto sin confundir revisión visual con medición contractual | S5, S9, S17 |
...
| SPEC-AVA-08 | Avance validado actualiza plan y habilita certificación | Reformular | Baseline/forecast existen, pero H5 sigue pendiente: una foto o revisión visual no crea ni aplica todavía un escenario Gantt y el certificado sigue separado | Una decisión humana explícita debe generar un escenario revisable con deltas contra baseline; una medición posterior inicia certificación. Nunca habilita pago automáticamente | S6, S9-S10 |

### Riesgo P0 de avance textual - cerrado localmente

Texto y audio accionables crean `OperationalProposal` con precondición, idempotencia y confirmación; el branch textual ya no modifica `task.progress` ni el Gantt directamente. La foto se vincula a `ProgressEvidence` y su lectura visual queda pendiente de revisión humana. El riesgo residual P1 es que los tres canales todavía terminan en registros de aprobación diferentes y no existe una transición aprobada desde esa revisión hacia el forecast; no deben presentarse como medición/certificación unificada.
...
| PRO-02 | Idempotencia, concurrencia y recuperación de proveedores | Parcial | Cobertura contractual amplia en WhatsApp; H2 conserva sólo `{ version, sessionId }` de forma durable, reconstruye bearer/enlace en memoria y usa fallback sin secreto si la sesión está stale. Sigue dispar en Stripe/otros y sin WABA real | S0, R0, S23 |
...
| PRO-05 | Retención, DSAR, backup/restore, RPO/RTO y borrado verificable | Ausente | La purga acotada de claims o sesiones no constituye un DSAR integral; faltan matriz de retención, cobertura de media/mensajes/identidad/derivados/backups y restore drill comprobado | Gate previo a datos laborales reales; S0, S23 |
...
| PRO-12 | Identidad laboral y destino de cobro protegidos | Parcial | El módulo valida CUIL/CBU/CVU/alias, conserva aviso versionado, cifra con AAD/keyring y serializa enmascarado. Ya existen API tenant-scoped, CAS, idempotencia, contrato/checks de titularidad y ledger append-only; sus migraciones se verificaron en Preview aislado. La verificación real no está operativa y la ruta falla cerrada con `503` sin un proveedor confiable | Completar consentimiento específico por destino/canal, Flow/UI, proveedor de titularidad y comprobante privado; nunca verificar, activar ni ejecutar pagos desde IA/WhatsApp | Piloto H3-H4, S14, R0 |
| PRO-13 | Rate limiting distribuido y WAF en fronteras públicas | Ausente como gate verificable | No hay evidencia de límites multi-instancia/WAF probados para webhook, `INIT`, captura/cancelación y descargas; un contador por proceso no protege un despliegue horizontal | Definir cuotas por IP/tenant/conexión/operación, reglas WAF, respuesta `429`, observabilidad y pruebas de ráfaga/degradación antes del piloto con trabajadores reales | R0, S23 |
