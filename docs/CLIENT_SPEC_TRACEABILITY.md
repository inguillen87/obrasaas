...
| PRO-12 | Identidad laboral y destino de cobro protegidos | Parcial local | El módulo valida CUIL/CBU/CVU/alias, conserva aviso versionado, cifra con AAD/keyring y serializa enmascarado. H4 ya incorpora consentimiento específico append-only por destino/canal, re-atestación legacy, panel CRM enmascarado, companion Flow de un solo uso, Data Endpoint terminal y reconciliador DB-only `UNCERTAIN → SUCCEEDED` que exige procedencia exacta y no reejecuta efectos. Sus cuatro migraciones `13000-13300` sólo pasaron PGlite local. Si la base no puede conciliar, el Data Endpoint devuelve `503` sin ciphertext y el journal queda inmediatamente reclamable, no bloqueado por una caché negativa de 24 horas. La verificación bancaria real falla cerrada con `503` sin proveedor | Completar orquestador de emisión, Neon y Vercel Preview, publicación/Meta E2E, proveedor de titularidad y comprobante privado; nunca verificar, activar, pagar ni reintentar resultados ambiguos desde IA/WhatsApp. No afirmar Production | Piloto H3-H4, S14, R0 |
