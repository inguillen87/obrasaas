...
| ID | Requisito original | Estado | Evidencia y decisión | Gap / criterio de aceptación | Secuencia |
| --- | --- | --- | --- | --- | --- |
| SPEC-PLN-01 | Gantt ideal con tareas, plazos y responsables | Parcial, motor en Preview y calendario quincenal local | `Task`/`TaskDependency` son la autoridad canónica para el Gantt y usan revisión CAS; baseline inmutable/versionada y forecast determinista ya existen. El read model local agrega quincenas civiles 1–15/16–fin de mes y exportación `.ics` autenticada. Las dependencias FS/SS/FF/SF y su lag se preservan, pero la UI sólo crea nuevas relaciones FS/0 | Aplicar/verificar la nueva migración en Neon Preview, cerrar smoke UI y `.ics`, agregar editor visual de tipo/lag y responsable por ID, y retirar el writer legacy sólo después de probar paridad/cutover. El `.ics` actual es snapshot de 90 días, no sync vivo | S3, S11, S15 |
...
| SPEC-MAT-02 | Pedido u orden a proveedor | Parcial local | Proveedores, OC y líneas canónicas existen. `SupplierCommitment` agrega localmente una fecha/ventana prometida para entrega o servicio, con vínculo opcional a OC aprobada/parcial y tarea canónica; no hay requisición/BOM, comparación de cotizaciones ni segregación completa | Aplicar/verificar la migración y UI en Preview; completar requisición → comparación/selección → OC aprobada y distribución cuantitativa usable por partida. Nunca compra automática | S11 |
...
| SPEC-MAT-04 | Registrar faltantes de entrega | Parcial local | El sistema calcula `PARTIALLY_RECEIVED/RECEIVED`, evita sobre-recepción por línea de OC y ahora modela localmente la fecha prometida, estados `TENTATIVE/CONFIRMED/AT_RISK/FULFILLED/CANCELLED` y recordatorio externo versionado | Falta detección automática de incumplimiento contra recepción, daño/exceso/rechazo, pendiente, owner, SLA, alerta interna a Compras/Director y evidencia Preview/Resend real | S11-S12, S15 |
...
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular, derivación local | Un compromiso vinculado a una tarea deriva readiness sin modificar `Task.status`. `FULFILL` material exige motivo y devuelve `ADMIN_ATTESTED`, nunca `AVAILABLE`, mientras no exista recepción cuantitativamente conciliada | Vincular BOM y cantidades de `GoodsReceiptLine` a compromisos, soportar cumplimiento parcial y validar en Preview. Foto/OCR sólo crea borrador y nunca habilita una tarea por sí solo | S12 |
...
| --- | --- | --- | --- | --- | --- |
| SOC-OPS-01 | Organizar tareas por quincenas y asociarlas a un calendario | Parcial local | La pantalla de Compras agrupa las tareas canónicas de los próximos 90 días y los compromisos en quincenas reales 1–15/16–fin de mes; el mismo read model entrega JSON y `.ics` autenticado. Declara truncamiento y el export falla cerrado si perdería datos | Aplicar en Neon Preview, recorrer UI autenticada e importar/reimportar en clientes soportados. No llamarlo sincronización: no hay feed revocable ni OAuth | S3, S15, R0 |
| SOC-PROC-01 | Registrar fecha prometida de material/aberturas o servicio y hacerla visible al equipo | Parcial local | `SupplierCommitment` registra proveedor, tipo, ventana, OC/tarea opcionales, estados, CAS, idempotencia, eventos y readiness derivado sin mutar la tarea | Migración/verificador en Neon Preview, smoke por rol y concurrencia; completar paginación y conciliación cuantitativa con recepciones | S11-S12, R0 |
| SOC-PROC-02 | Enviar al proveedor un email una semana antes de ejecutar el servicio | Parcial local, externo pendiente | Outbox dedicado, cron cada 15 minutos, envío Resend con idempotencia persistida, estados inciertos fail-closed y webhook firmado. La UI fija siete días; la marca de email es atestación de Administración, no doble opt-in del proveedor | Verificar dominio/remitente, webhook, cron y ciclo accepted/delivered en staging; probar timeout, 429, bounce, complaint, supresión, reprogramación y owner de incidentes antes de Production | S15, R0 |
...
| SPEC-ALT-03 | Material no llegó → Compras + Director | Parcial local | OC y recepción parcial/completa existen; `SupplierCommitment` agrega fecha prometida y riesgo derivado, y el outbox local avisa al proveedor. Todavía no existe alerta interna durable a Compras/Director ni detección automática de incumplimiento por cantidades recibidas | Comparar fecha/cantidad comprometida con recepciones reales, crear un único evento interno deduplicado con owner, resolución y escalamiento, y validar el email externo en Resend real | S11-S12, S15 |
...
| PRO-02 | Idempotencia, concurrencia y recuperación de proveedores | Parcial local | Además de WhatsApp, los recordatorios de proveedores usan outbox, clave Resend persistida, claim atómico, bloqueo/fence de obra y webhook append-only. Un dispatch ambiguo pasa a `UNCERTAIN` y no se reintenta a ciegas; conflicto, bounce, complaint, failure y supresión requieren tratamiento explícito. Sigue sin prueba Resend/Vercel real y dispar en Stripe/otros | S0, R0, S15, S23 |
...
| PRO-04 | Observabilidad, SLO, alertas y runbooks | Parcial local | El cron de recordatorios expone métricas sanitizadas y estado `healthy/degraded`; existe [runbook específico](./SUPPLIER_COMMITMENTS_AND_FORTNIGHT_CALENDAR.md). No hay aún alertas conectadas, SLO ni evidencia de ejecuciones Vercel/Resend reales | S0, S15, S23 |
