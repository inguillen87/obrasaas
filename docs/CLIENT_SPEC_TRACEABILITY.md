...
| SPEC-MAT-03 | Recepción con foto y remito firmado | Parcial en Preview | `GoodsReceipt` admite líneas parciales exactas, impide sobre-recepción, actualiza la OC y vincula remito privado. El saldo server-owned agrega todo el historial `POSTED` de las órdenes visibles, sin reconstruirlo desde las últimas 500 recepciones | Completar aceptado/rechazado/dañado, firma jurídica, retención, proveedor/receptor visibles, conciliación con compromiso y E2E autenticado | S12 |
| SPEC-MAT-04 | Registrar faltantes de entrega | Parcial en Preview; Resend pendiente | El sistema deriva exactamente recibido y pendiente por partida de OC, calcula `PARTIALLY_RECEIVED/RECEIVED` y modela fecha prometida, riesgo y recordatorio externo versionado | Falta comparar automáticamente cantidad comprometida contra recepción explícitamente asignada, además de daño/exceso/rechazo, owner, SLA, alerta interna a Compras/Director y evidencia Resend real | S11-S12, S15 |
...
| SOC-PROC-01 | Registrar fecha prometida de material/aberturas o servicio y hacerla visible al equipo | Parcial en Preview | `SupplierCommitment` registra proveedor, tipo, ventana, OC/tarea opcionales, estados, CAS, idempotencia y eventos. La UI ya captura una partida de la OC y su cantidad exacta; la disponibilidad continúa fail-closed y no muta la tarea | Completar smoke por rol, UI multi-partida, paginación y asignación explícita e inmutable `GoodsReceiptLine → SupplierCommitmentLine` | S11-S12, R0 |
...
| PRO-03 | CI, migraciones gobernadas y E2E por rol | Parcial en Preview | El deployment `dpl_BgdEVh9n3wJunmvrMSXw9GCBCaSK` ejecutó un preflight read-only del alcance de proyecto del outbox antes de `prisma migrate deploy`, encontró 0 filas incompatibles y luego verificó las 111 migraciones. Build y smokes sin sesión pasaron; smoke autenticado portable y E2E core siguen abiertos | S0 |
