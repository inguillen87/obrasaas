...
| SPEC-MAT-05 | Stock actualizado | Parcial local; Preview pendiente | S12.2A agrega catálogo/ubicación tenant-scoped, vínculo inmutable de línea, putaway atómico desde todas y sólo las disposiciones `ACCEPTED`, ledger append-only, reversión espejo y balance on-hand DB-owned. No backfillea el JSON legacy ni acepta cantidad/ubicación del cliente | Verificar migración y UI autenticada en Preview; agregar consumo, devolución, transferencia, ajuste y cutover legacy con rollback | S12 |
| SPEC-MAT-06 | Recepción marca tarea “con materiales disponibles” | Reformular; aceptación en Preview, on-hand local, `AVAILABLE` cerrado | Un compromiso vinculado a una tarea deriva readiness sin modificar `Task.status`. La recepción `ACCEPTED` sólo pasa a existencia física tras putaway explícito; S12.2A todavía no reserva contra una BOM. `FULFILL` administrativo sigue siendo `ADMIN_ATTESTED`, nunca `AVAILABLE` | Implementar BOM versionado y reserva/liberación exacta. Sólo cantidad aceptada, on-hand y reservada suficiente puede derivar `AVAILABLE`; foto/OCR nunca habilita por sí solo | S12 |
