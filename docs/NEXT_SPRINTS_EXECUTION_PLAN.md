...
## Corte base: 2 de agosto de 2026 · actualización PRO-05A.1: 10 de agosto de 2026
...
- el commit `edeea82` quedó `Ready` en Vercel Preview y el alias estable fue cortado al mismo artefacto. El release detectó 119 migraciones sin pendientes, aprobó la reconciliación estructural y ejecutó el verificador PRO-05A contra Neon antes del `prisma generate` gobernado y del build Next 88/88. CI PostgreSQL 17 también lo exige y cerró sus tres jobs. El [corte PRO-05A.1](./evidence/2026-08-10-preview-edeea82.md) acredita ese gate y un smoke AUDITOR acotado; no acredita DSAR ejecutable, matriz completa de roles, cross-tenant, Meta real, S12.2C ni Production;
...
- la migración aditiva `20260729140000_data_subject_discovery_foundation` aplica desde cero junto con las 108 anteriores en PGlite. Su verificador conductual prueba tenant scope, transacción read-only, hash UTC, cobertura fail-closed, consistencia terminal y append-only; ya quedó verde en CI PostgreSQL 17 y Vercel/Neon Preview sobre el esquema completo de 119 migraciones. Falta recorrer el endpoint PRO-05A con ADMIN sintético autorizado y completar la matriz de roles/cross-tenant; no hay evidencia de Production;
...
6. **siguiente gate P0, S12.2C:** reserva/liberación exacta contra una revisión de BOM. Sólo cantidad aceptada, on-hand y reservada suficiente puede derivar `AVAILABLE`;
7. consumo, devolución, transferencia y ajuste aprobable antes de retirar el snapshot legacy de acopio;
8. alerta interna durable a Compras/Director cuando fecha y cantidad aceptada incumplen la promesa;
9. smoke autenticado, Resend real y E2E por rol antes de Production. S12.1, S12.2A y las migraciones 116+117 de S12.2B ya pasaron sus verificadores separados en Preview. Esos gates no sustituyen el recorrido autenticado.

Sigue faltando la requisición previa a la BOM, además de reserva, consumo, transferencia, ajuste, cotización/selección, evidencia fotográfica tipada/firma de recepción y UI multi-partida. La ampliación de la socia ya está cubierta como base por fechas de material o servicio, calendario civil por quincenas 1-15/16-fin, snapshot `.ics` y recordatorio configurado por defecto siete días antes. No es sincronización viva y el envío real sigue gated por Resend, dominio/remitente verificado, cron/webhook observados y E2E. Contratos: [ledger S12.2A](./INVENTORY_STOCK_LEDGER.md) y [BOM S12.2B](./TASK_MATERIAL_REQUIREMENTS.md). Evidencias remotas: [inspección/faltantes `b0ba0f8`](./evidence/2026-08-02-preview-b0ba0f8.md), [ledger on-hand `4760a50`](./evidence/2026-08-02-preview-4760a50.md) y [DB/build S12.2B `054a82c`](./evidence/2026-08-09-preview-054a82c.md). Ninguna sustituye el recorrido autenticado ni acredita Production.
