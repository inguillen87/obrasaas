...
- S12.2B está implementado sólo en el worktree local: BOM/requerimiento por tarea versionado, inmutable y explícito, incluida la declaración `NO_MATERIALS_REQUIRED`. La migración `20260802180000_task_material_requirements`, API, UI y verificador existen y 27 pruebas focalizadas pasan; todavía no hay Neon/Vercel Preview ni journey autenticado para este corte. No crea reservas y todos sus estados conservan `available: false`;
...
5. **código local; Preview pendiente:** S12.2B publica una cadena inmutable de requerimientos/BOM por tarea, o `NO_MATERIALS_REQUIRED`, con scope server-owned, CAS, idempotencia, snapshots y cantidades exactas. `SupplierCommitment` se muestra sólo como **PROMESA, NO RESERVA**;
