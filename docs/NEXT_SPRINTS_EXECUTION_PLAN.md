...
- H4 tiene una base local avanzada: consentimiento específico append-only por destino/canal, re-atestación segura de registros legacy, panel CRM sólo enmascarado, Flow de cobro, companion de sesión de un solo uso, Data Endpoint terminal, replay exacto y fence `UNCERTAIN`. El cron autenticado recupera un `PROCESSING` vencido sólo hacia `UNCERTAIN`, sin reenviar. El reconciliador DB-only puede avanzar `UNCERTAIN → SUCCEEDED` únicamente cuando encuentra un destino ya persistido ligado por tenant, reserva, HMAC, tipo/fingerprint, claves de operación, canal y consentimiento exactos; no recibe el formulario ni invoca bridge, proveedor, pago, WhatsApp o IA. Las cuatro migraciones `13000/13100/13200/13300` aplican juntas en PGlite descartable, pero todavía no fueron aplicadas ni verificadas en PostgreSQL real/Neon Preview;
- cierre H4: el caller inbound especializado ya existe localmente y emite la sesión genérica más su companion en la transacción del webhook sólo para identidad `CANONICAL` verificada; el POST a Meta queda fuera de esa transacción y conserva el journal anti-duplicado. Falta coordinar el cutover legacy antes de migrar; verificar migraciones, cron y reconciliación en Neon; desplegar y validar el build H4 en Vercel Preview; publicar plantilla/Flow en Meta; recorrer E2E; integrar un proveedor confiable de titularidad; y emitir el comprobante por enlace privado. Una sesión sin resultado persistido o con procedencia inconsistente permanece `UNCERTAIN` para resolución auditada; nunca se verifica, activa, paga ni se reintenta un resultado ambiguo automáticamente desde IA o WhatsApp. Nada de esto acredita Production.
