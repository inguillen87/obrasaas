...
Esta base aún no es un journey operativo: `issueWorkerPaymentFlowSession` no tiene caller de producto y el catálogo proactivo no envía cobro, por lo que hoy no se puede abrir este Flow desde un WhatsApp real. Faltan el orquestador especializado, publicación de plantilla/Flow en Meta, E2E, proveedor confiable de titularidad —la verificación sigue cerrada con `503`— y comprobante privado.
