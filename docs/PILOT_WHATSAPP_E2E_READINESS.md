...
- La rama `codex/platform-ux-foundation` usa una rama Neon aislada. En el despliegue Preview del commit `d6b29b9` se detectaron 97 migraciones, se aplicaron las dos nuevas y pasaron todos los verificadores de migración; el build remoto quedó `Ready`. Los IDs, timestamps, comandos y smokes sanitizados están en la [evidencia reproducible del corte](./evidence/2026-07-28-preview-d6b29b9.md). Production no fue modificada.
...
- El alias estable `https://obrasaas-preview.vercel.app` ya apunta al redeploy `Ready` de `d6b29b9` y respondió correctamente en los smokes públicos y de rechazo del webhook. Falta ingresar `META_APP_SECRET`, generar un token temporal nuevo en Meta y redesplegar antes del recorrido real; el panel de Meta mantiene una reautenticación humana pendiente y ninguna contraseña se captura ni automatiza.
