...
- H4: opt-in opcional de constancia ligado al HMAC y persistido en la companion, registro enmascarado con acceso de 15 minutos, descriptor durable sin secreto, bearer reconstruido sólo después del claim de entrega, enlace con token en fragmento, webview que lo elimina antes del primer request y PDF dinámico sin storage. La constancia nunca muestra el destino completo y no acredita titularidad, validación bancaria, activación, transferencia ni pago. Meta/WhatsApp y el dispositivo destinatario necesariamente procesan el mensaje con el enlace; un reenvío o captura transfiere el bearer mientras esté vigente, por lo que el riesgo residual debe aceptarse explícitamente y probarse junto con revocación, límite de cinco accesos y copy de no compartir.

No debe presentarse todavía como completo: la cadena H2 local foto → opción de geolocalización u opt-out → evidencia está implementada por contrato y pruebas, pero las migraciones `20260729100000_progress_evidence_location_capture` y `20260729110000_progress_evidence_location_rate_limit` aún no fueron aplicadas en Neon Preview y no se recorrió una foto entrante real de Meta ni mensajería bidireccional E2E sobre el tenant. La geolocalización corrobora contexto; puede falsificarse y no prueba identidad ni presencia física. H5 ya conecta localmente una revisión humana aprobada/corregida con una observación append-only y un forecast determinista visible contra la baseline; no lo hace automáticamente ni muta el plan. Su migración y journey autenticado todavía no fueron verificados en Preview. H3.1 ya incluye el Flow especializado y la pantalla CRM; siguen pendientes el smoke UI/runtime, observar el cron y el E2E de Meta. H4 incorpora localmente caller inbound, sesión base/companion atómica, reconciliador DB-only `UNCERTAIN → SUCCEEDED` y constancia privada por opt-in. Las 108 migraciones y el verificador H4 pasan juntos desde cero en PGlite descartable, pero todo el delta `13000-13400` sigue sin smoke en PostgreSQL real/Neon Preview, Vercel Preview ni Meta E2E. También faltan el reenvío fuera de ventana mediante plantilla `UTILITY` y un proveedor confiable de titularidad bancaria.
...
Base previa disponible: validación estricta de CUIL, CBU, CVU y alias; cifrado AES-256-GCM con AAD; keyring rotatable; fingerprint HMAC por tenant; API autenticada tenant-scoped; revisión maker-checker-activator y ledger append-only. El incremento H4 actual agrega localmente consentimiento específico por destino/canal, re-atestación legacy, panel CRM sólo enmascarado, definición Flow, companion de sesión de un solo uso, Data Endpoint terminal y [constancia privada](./WORKER_PAYMENT_PRIVATE_RECEIPT.md). Sus estados normales son `OPEN → PROCESSING → SUCCEEDED|UNCERTAIN`; la migración `13300` agrega sólo la recuperación `UNCERTAIN → SUCCEEDED` mediante procedencia exacta ya persistida y reloj de PostgreSQL. El reconciliador compara tenant, reserva, HMAC, tipo/fingerprint del destino, claves de operación, actor/canal y consentimiento sin leer valores financieros y sin invocar bridge, proveedor o WhatsApp. Las cinco migraciones `20260729130000` a `20260729134000` pasan juntas en PGlite descartable y permanecen locales; todavía no se aplicaron ni verificaron juntas en PostgreSQL real/Neon Preview ni en un deployment H4 de Vercel.

El Flow ofrece `receipt_delivery_requested` como opt-in opcional. Sólo `true` se incorpora a la forma canónica, al HMAC y a la companion; `false` o ausencia conservan compatibilidad y no emiten constancia. Después de `SUCCEEDED`, la constancia persiste únicamente referencia opaca, datos enmascarados, integridad y compromiso del token. El acceso vence a los 15 minutos; el token en claro no se almacena, se reconstruye sólo después de ganar el claim durable de entrega y viaja en el fragmento `#token=...`. La webview elimina ese fragmento antes del primer request y conserva el bearer sólo en memoria. El PDF se genera dinámicamente y no se guarda en storage. El CBU, CVU o alias completo nunca entra al descriptor, enlace, webview, PDF ni metadata de auditoría de entrega.
...
El journey ya comienza localmente desde un texto inbound: la intención `PAYMENT_DESTINATION` abre el Flow sólo para una identidad `CANONICAL` con persona/canal verificados, obra activa y referencia publicada. La sesión base y companion quedan confirmadas antes de aplicar el evento; datos financieros pegados en el chat se redactan antes de la cola. Esto todavía no lo vuelve operativo en un WhatsApp real: el catálogo proactivo genérico no envía cobro y siguen pendientes despliegue H4, publicación de Flow en Meta, E2E, proveedor confiable de titularidad —la verificación sigue cerrada con `503`— y el reenvío fuera de la ventana de atención mediante una plantilla `UTILITY` aprobada.
...
Este es el recorrido objetivo de aceptación H6; forecast y constancia privada están disponibles sólo localmente. Ninguno está certificado todavía como journey externo y la constancia no convierte la recepción de un destino en pago.
...
9. El destino de cobro se recibe para revisión por el circuito autorizado y Carlitos recibe un enlace privado a la constancia si dio opt-in; el documento nunca muestra el dato completo ni acredita pago o validación bancaria.
